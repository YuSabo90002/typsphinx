...
# colliding with a real docname's own output path. Unlike
# derived_docname_collision_gate, the collision here comes from an explicit
# `typst_documents` target name naming an existing docname ("chapter1") as
# the master's target -- not from the derived-default project-name
# slugification. `project` is deliberately neutral (non-colliding) so the
# only source of the collision is the explicit entry.
#
