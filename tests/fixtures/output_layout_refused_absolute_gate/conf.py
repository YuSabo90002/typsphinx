...
# refused-target (absolute path) worked example.
#
# This fixture's ONE job: be the built proof for the absolute-target
# refusal shape ("Targets that are refused") on
# docs/source/user_guide/output_layout.rst -- a literal copy of
# 51-RESEARCH.md Part C build 3b (measured 2026-08-14, sphinx-build -b
...
# the drive-qualified shape both fall back to the SAME basename ("manual");
# a single project holding both would trip the self-collision abort instead
# of exercising the fallback each is meant to prove.
...
# Part C build 3b. An absolute target, refused with a warning and a
# fallback to its basename ("manual").
typst_documents = [("index", "/abs/manual", "Title", "Author", "typst")]
