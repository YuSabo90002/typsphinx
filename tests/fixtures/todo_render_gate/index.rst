...
the ``.. todo::`` body below must render inside a gentle-clues ``task()``
box with its own dynamic title (D-01), which must appear in the compiled
PDF text exactly once -- sourced only from the box's own title child,
never from this heading or the project title.
