...
- [ ] **CI-02**: A `typst compile` smoke test is added that would catch a `kai`-class `@preview` break before release (closes the gap that the internal-only sync test misses)
