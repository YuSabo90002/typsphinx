...
- [x] **GATE-02**: Sphinx's own `doc/` tree compiles end-to-end through `typstpdf` with no fatal `TypstCompilationError`; the remaining `unknown_visit` warnings are catalogued by frequency and the empty-URL warning-count reduction is measured before/after
