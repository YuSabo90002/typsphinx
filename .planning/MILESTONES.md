...
itself. **5/5 v1 requirements complete.**

**Delivered:** `doctest_block` finally has a handler. A `>>>` example is not a directive an author
can opt out of — docutils' parser auto-detects any `>>>`-led block — so every Google/NumPy-style
`Examples:` docstring produced a node `TypstTranslator` had none for. It fell through
`unknown_visit()` while its `Text` children were still re-emitted as inline text, so the example
collapsed onto one line, and where it sat beside other content in the same container the emission
was malformed enough that Typst refused the file and `-b typstpdf` wrote no PDF for the affected
masters. `visit_literal_block` / `depart_literal_block` now accept `nodes.doctest_block` and supply
the language tag themselves, because `HighlightLanguageTransform` assigns `node['language']` only to
`literal_block`. The gate drives a real `typst.compile()` over both the plain-paragraph and the
definition-list/field-body context, each recorded observably RED against the pre-handler tree first.
In the same clean build, the docutils `Unexpected indentation` and `Block quote ends without a blank
line` messages this project's own docstrings raised are gone: `sphinx-build -b typst docs/source`
goes from `build succeeded, 5 warnings.` to `build succeeded.` with zero warnings, and neither
message class may rise while the other falls.

**Published.** `typsphinx 0.9.6` is on PyPI — wheel (194,339 B) and sdist (860,244 B) — and it
carries three completed but never-published milestones with it: v0.9.3's NixOS toolchain repair,
v0.9.4's builtin-generics migration, and v0.9.5's documentation link check, all promoted into one
`## [0.9.6]` CHANGELOG heading. `0.9.3`, `0.9.4` and `0.9.5` join `0.9.1` as permanently unclaimed
on PyPI. REL-16 ends a five-milestone run of declining a `### Known Limitations` section: `[0.9.6]`
discloses NUM-01 with its precondition, both symptoms and a workaround. The candidate set was
narrowed from three defects to one on measurement — the other two had been fixed in v0.8.0, and
disclosing a closed defect would have been a false statement in a release note.

**Publish record.** Branch `gsd/v0.9.6-doctest-block-rendering-and-release` merged to `main` as
`6fcc5adb` via **PR #156**, 15/15 checks green including both `windows-latest` and both
...
never run on a v0.9.x tag before**, which REL-15's own text called out as unproven. The Release
body's first 100 lines are byte-identical to `scripts/extract_changelog_section.py 0.9.6` (7104
bytes, digest `f9a52e3d…`), with zero `### Planned for Future Releases` leakage; the remaining 32
lines are GitHub's own Installation and What's Changed sections, the same shape v0.9.2 published.
`typsphinx-doc-translations` `update-pin.yml` run `35735922806` advanced its submodule pin to
exactly `6fcc5adb` and resynced the ja catalogs (`158fa1c4`); that repository is tagged `v0.9.6` as
a separate action. Read the Docs `stable` was **re-measured** on both projects rather than assumed —
`en` at `6fcc5adb`, `ja` at `158fa1c4`, both rendering `typsphinx 0.9.6`.

**Closeout:** `override_closeout`, on a real `v0.9.6-MILESTONE-AUDIT.md` (`status: tech_debt`;
requirements 4/5 satisfied with REL-15 held by design, phases 2/2 verified, integration 6/6, flows
3/3, zero gaps in all three buckets). The override has one cause and it is the same one as the last
three closes: `init.manager` reported both phases `verification_status: stale` /
`phase_complete: false` while both VERIFICATION.md files read `status: passed` (74 at 4/4, 75 at
5/5, zero overrides applied), because `.planning/` tracking commits land after each verification and
touch files its fingerprint covers. Neither phase's own work changed, and the same-day audit
re-checked both. **Known verification overrides: 0 newly acknowledged, 11 carried forward from a
prior close** (see STATE.md Deferred Items) — the pre-close artifact audit found 2 open todos and
**both were fixed rather than acknowledged**, so the re-run reported all artifact types clear.

**Three things worth carrying.**

1. *A deferral's stated reason can lapse before the deferral is revisited.* Both open todos had been
   held back by Phase 75's prep-only fence, whose cost argument was that touching `typsphinx/` or
   `conf.py` would force SC4's entire green proof to be re-taken on a changed tip. By the time the
   close ran, `origin/main` had moved (Dependabot #152–#155) and `main`'s protection is `strict`, so
   that re-take was mandatory anyway. The premise was gone; the todos were closed inline as quick
   tasks `260922-tbe` and `260922-tkt` for no additional CI cost.
2. *A todo's own proposed fix was a silent no-op, caught by reading the dependency's source.* The
   PyPI-anchor todo asked for a `linkcheck_anchors_ignore_for_url` entry covering
   `https://pypi.org/project/typsphinx/#history`. Sphinx's `_check_uri` partitions the URI on `#`
   **before** matching those patterns, so a pattern carrying the fragment can never match — and a
   bare `pypi\.org` entry would prefix-match every PyPI project page. The landed pattern is
   end-anchored at the bare project URL, and its suppression set was proved by probe to be exactly
   one anchor out of 41 rather than argued from the regex.
3. *The result is this project's first unconditionally clean `tox -e linkcheck`: 96/96 `working`,
   exit 0.* The two `v0.9.6` tag URLs resolved the moment the tag existed, exactly as
...
milestones without being scoped), WR-02, WR-03, LNK-01, NUM-01 (now disclosed, still unfixed), and
Phase 75's `VALIDATION.md` at `status: draft` — the third consecutive release-prep phase to skip
`/gsd-validate-phase`.
...
machine again, and dependabot PRs are tested instead of dying at the install step. Before this
milestone, `ruff`, `tox -e py312`, `.venv/bin/uv` and tox's downloaded CPython all failed on NixOS'
stub loader, and every dependabot PR bumped `pyproject.toml` without `uv.lock`, so all eleven
`uv sync --locked` steps refused it before a single test ran. **Nothing under `typsphinx/` changed**
and nothing was released: no tag, no PyPI upload, no GitHub Release, `pyproject.toml` still at
`0.9.2`. The CHANGELOG bullets wait under `## [Unreleased]` for the next published release.

**Closeout:** override_closeout, on a real `v0.9.3-MILESTONE-AUDIT.md` (`status: tech_debt`,
requirements 20/21 before the close, phases 6/6, integration 7/7, flows 4/4). The override has one
cause: `init.manager` reported all six phases `verification_status: stale`. Each VERIFICATION.md
fingerprints its `covered_files`. Phases 64–67 list `.planning/REQUIREMENTS.md`, which later phases
legitimately updated. Phase 68's review-fix commit landed after its verification. Phase 69 covers
`CHANGELOG.md`, whose wording the owner had fixed after the audit. The owner chose not to re-run
`/gsd-verify-work` six times, because the same-day audit had re-checked every phase.
Known verification overrides: **2 newly acknowledged, 11 carried forward from a prior close** (see
STATE.md Deferred Items). The two new ones are the `add-sphinx-linkcheck-ci-job` todo (raised for the
seventh consecutive close) and `SEED-005` (GSD workstreams, planted during this milestone).
**Phases:** 6 (64–69) · **Plans:** 27 · **Tasks:** 70
**Requirements:** 21/21 v1 requirements complete; REL-12 checked at this close, after the merge was observed
**Timeline:** 2026-09-02 → 2026-09-13 (12 days)
**Git:** milestone branch `gsd/v0.9.3-toolchain-and-dependency-update-repair` (238 commits before the
close) took `origin/main` (`cf3305ce`) in as a `--no-ff` merge, then merged to `main` as
``58d578f2`` via **PR #143**, 15/15 checks green including both `windows-latest` and both `macos-latest` lanes. Merge commit, not squash. No tag. Branch kept.
Two earlier PRs from inside the milestone also landed on `main`: **#137** (the `dependabot.yml`-only
ecosystem switch, Phase 66) and **#138** (dependabot's first `uv` PR, `ruff` 0.16.6, the proof,
Phase 67). #123 and #128 were closed on their merits.
**Code delta (milestone scope, excl. `.planning/`):** 10 files, +329 / −91 lines. `typsphinx/`: 0.
`flake.nix` +144, `tests/test_toolchain_config_gate.py` rewritten for the `tox-uv` pin, and
`CLAUDE.md` / `tox.ini` / `CHANGELOG.md` / `.github/dependabot.yml` / `pyproject.toml` / `uv.lock`.
**Zero runtime dependencies added**, no new `typst_*` config value, no `@preview` change, CI
workflows unchanged.

**Key accomplishments:**

- **Every documented command runs on NixOS through FHS shims** (NIX-01..NIX-08, Phase 64). `flake.nix`
  keeps `mkShell` as the devShell and puts seven bare commands on PATH (`uv`, `tox`, `ruff`, `black`,
  `mypy`, `pytest`, `sphinx-build`). Each shim walks up to the checkout's own `.venv/bin/<tool>` and
  runs it inside a Linux-guarded `buildFHSEnv`. A missing tool fails loudly with exit 127 instead of
  running some other binary. `ruff --version` reported the `uv.lock` version, not nixpkgs'. The first
  re-measurement found `libz.so.1` missing for uv-managed interpreters (3 of 7 gates failed). A
  gap-closure plan added `zlib` to the FHS closure, and all gates were re-measured clean in a
  genuinely relaunched session.

- **`tox-uv-bare` → `tox-uv`, on the uv tox actually resolves** (TOX-01..TOX-04, Phase 65). A
  four-file commit with `uv.lock` regenerated by `uv lock`. The roadmap's claim that tox-uv's
  discovery was "a different code path" was falsified against the installed source before planning:
  the bundled step *is* `uv.find_uv_bin()`, which resolves the same `.venv/bin/uv` the `uv` shim
  does. The claim was put to the owner and closed with an `AMENDED` block. CI run `34681968010` was
  12/12 green.

- **Dependabot maintains `uv.lock`, proven on a real PR** (DEP-01..DEP-05, Phases 66–67).
  `package-ecosystem: "pip"` → `"uv"` landed on `main` through its own PR (#137), because dependabot
  reads its config only from the default branch. That fact made "one PR to `main`" unreachable, and
  the constraint was amended with owner approval. The proof is dependabot's own #138 (`ruff`
  0.16.6), which updated `pyproject.toml` and `uv.lock` in one commit and ran the full CI, including
  the test, lint and type jobs. #123 was closed as superseded by #138. #128 was closed because Sphinx
  9.1.0 still caps `docutils<0.23`.

- **The grouped-update path is recorded as unproven, not passed** (DEP-03 / Phase 67 SC#4). No
  `sphinx-typst-stack` group PR has opened under `uv`: its `docutils` member hits
  `dependency_file_not_resolvable` against the same Sphinx cap. The milestone does not claim
  grouping works under `uv`. The CHANGELOG bullet was reworded after the audit to stop implying it
  carries over unchanged.

- **Documentation describes what landed** (DOC-19..DOC-21, Phase 68). `CLAUDE.md` gained a "NixOS
  development shell" subsection and a worktree boundary: `direnv` never loads inside a worktree, so
  the per-worktree `uv sync` recipe is unchanged. The recipe was proven byte-identical to base.
  `tox.ini` and the two test files lost their current-tense `tox-uv-bare` rationale; the comment-only
  change was proven by masked-AST-hash equality. `flake.nix`'s comment rewrite was proven identical
  on all four declared systems via `drvPath`, and it states that darwin is unverified by construction.

- **The release-checkbox fence held again, and REL-12 closed on observation** (Phase 69 + this
  close). `phase.complete` flipped REL-12 for the eighth time at a release-prep close. The flip was
  reverted before commit and the SHA-256 re-matched `02cb9deb…`. At this close the operator merged
  PR #143 and then observed five facts before checking the box: the merge commit on `origin/main`,
  `pyproject.toml` still `0.9.2`, no `v0.9.3` tag, PyPI 404 for `0.9.3`, and no `v0.9.3` Release.

**Known limitations:** `flake.nix` is load-bearing with zero CI coverage, and its darwin branch cannot
be exercised from Linux (accepted, owner decision 2026-09-02). A cold `@preview` cache / TLS
through the FHS sandbox has never been measured (64 WR-01, `cacert` not in `targetPkgs`). Nyquist
VALIDATION.md is still `draft` in all six phases, and no SECURITY.md was produced although
`security_enforcement` is on. Four dependabot `uv` PRs (#139–#142) were left open for ordinary
maintenance.
