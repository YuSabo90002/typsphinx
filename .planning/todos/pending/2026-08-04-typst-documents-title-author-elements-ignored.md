...
## 【2026-08-04 更新】v0.7.1 に復帰 — 下記の「スコープ外である理由」は撤回済み

オーナー判断 2026-08-04（Phase 45.1 作成後）: **D-02 の先送りを覆し**、この項目は v0.7.1 内の
**Phase 44.2 / 要件 CONF-09** として実施する。判断が変わった理由は
`docs/source/user_guide/templates.rst:189` が `title` を「from `typst_documents`」と説明しており、
配線が無いままだと Phase 45.1 の SC#2（公表契約と実挙動が両方向で一致）が「食い違いを文書化する」
方向にしか閉じられなかったこと。パッチリリースに2件目の user-visible 変更が乗るコストは
回避ではなく**受容**され、Phase 46 の CHANGELOG が両方を明記する。

以下の節は当時の判断の記録として残す（現在は無効）。

## ~~Phase 44 のスコープ外である理由~~（撤回済み）
