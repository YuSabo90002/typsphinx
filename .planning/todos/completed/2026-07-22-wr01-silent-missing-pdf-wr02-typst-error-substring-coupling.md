...
closed: 2026-07-22
closed_by: Phase 22.3 (typstpdf-builder-warning-hardening)
status: resolved
...
Closed by **Phase 22.3**. Both warnings are delivered and independently verified.

- **WR-01** — resolved **behavioral** (CONTEXT.md D-01). Both skip branches in
  `TypstPDFBuilder.finish()` now `failures.append(...)`; the aggregate `ExtensionError`
  wording was widened past "failed to compile to PDF" (D-02); the missing-`.typ` message
  splits on `env.found_docs` (D-04) with the fallback text kept byte-identical so
  `tests/test_target_name_render_gate.py:122` stays meaningful.
- **WR-02** — all six `typst-py` error-text assertions removed from
  `tests/test_nested_master_render_gate.py` (D-10), replaced by structural preconditions
  plus a three-part ablation whose GREEN half produces real PDF bytes (D-08/D-09).

**Coordinate correction:** this todo's `files:` block cites `builder.py:895-897` and
`850-856`. Phase 22.3 research measured both as **stale** — the real coordinates were
`922-924` (missing-`.typ` guard) and `868-886` (docstring); `895-897` was an unrelated
whole-project-empty early return. Left above as written for the historical record.

**Follow-on filed, not fixed:** the non-string-docname raw `TypeError`
(`.planning/todos/pending/2026-07-22-non-str-docname-typeerror-in-typstpdf-finish.md`)
was deliberately deferred by D-06.

**Found and fixed during the phase's own code review (CR-01):** `TypstWriter._is_master_document()`
indexed `doc_tuple[0]` unguarded, crashing the *write* phase for a malformed entry in any
multi-document project — before `finish()` ran, so no PDF was produced at all. Fixed in
`8da32c3`; see `22.3-REVIEW-FIX.md`.

Verification: `22.3-VERIFICATION.md` — 5/5 ROADMAP success criteria verified,
24/25 must-haves (one `verification: backstop` concurrency truth correctly abstained).
