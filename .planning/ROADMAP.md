...
**No milestone is active.** v0.9.4 completed 2026-09-13 and was merged to `main` via PR #145 with
no tag, no PyPI upload and no GitHub Release; `pyproject.toml` stays at `0.9.2` and `## [Unreleased]`
now carries four bullets from two unpublished milestones (v0.9.3's three and v0.9.4's one). The
version numbers `0.9.3` and `0.9.4` are both unclaimed, not decided — the next milestone's scoping
settles which one the next published release uses. Start the next milestone with
`/gsd-new-milestone`; phase numbering continues at **Phase 72**.
...
(never resets to 1). v0.9.4 ran Phases 70–71, so the next milestone starts at **Phase 72**.
...
No active milestone. Phases 1–71 shipped or completed across v0.4.4 → v0.9.4; their per-phase plan
counts, statuses and completion dates are preserved in each milestone's archived roadmap under
`milestones/`.

## Roadmap Evolution

Per-milestone evolution notes are archived with their milestone. v0.9.4's — the two-phase structure
below the `standard` granularity range, DOC-22 and DOC-23 mapped to Phase 70, the todo move landing
with the ignore flip rather than with the `CLAUDE.md` rewrite, and the REL-13 AMENDED block
withdrawing the ja-catalog clause — live in [milestones/v0.9.4-ROADMAP.md](milestones/v0.9.4-ROADMAP.md).
...
**Todo closed by v0.9.4** (2026-09-13): `2026-07-22-modernize-typing-imports-drop-up006-up035-ignore`
(Phase 70, QUA-09 / QUA-11 / QUA-12 / DOC-22), moved to `todos/completed/` in the commit that removed
the ignores.

**Still open and deferred** (the 5 pending todos, carried out of v0.9.4):
...
- `2026-09-13-doctest-block-unhandled-collapses-examples-to-one-line` (TRN-01, `severity: major`) —
  ...
  2026-09-13 during the Issue #91 re-measurement; acknowledged at the v0.9.4 close.
...
- `2026-08-29-hardcoded-delimiter-path-fragments-in-translator-relative-path-debug-logs` (MSG-06,
  ...
  `quote_path()`, which now exists. Adjacent to Phase 70 and deliberately not absorbed by it.
...
- `2026-07-22-add-sphinx-linkcheck-ci-job` (Future QUA-08 / LNK-01) — deferred again; a CI job.
  Raised at seven consecutive closes (acknowledged at the v0.9.3 close, still suppressed).
...
milestone across seven consecutive scopings. **`SEED-005-gsd-workstreams-for-parallel-roadmap-tracks`**
...
through v0.9.3 and v0.9.4 because neither changed product behaviour: WR-02's `confdir` gap, the
tripled "Custom template not found" warning, and NUM-01's `numref` per-master divergence. The
`### Known Limitations` decision stays open for the next *published* release.
...
---
*Roadmap created: 2026-07-04 · Reorganized at each milestone close: v0.4.4 (2026-07-05), v0.5.0 (2026-07-11), v0.6.0 (2026-07-13), v0.6.1 (2026-07-19), v0.6.2 (2026-07-23), v0.6.3 (2026-07-25), v0.6.4 (2026-07-28), v0.6.5 (2026-07-29), v0.7.0 (2026-08-04), v0.7.1 (2026-08-11), v0.8.0 (2026-08-15), v0.9.0 (2026-08-22), v0.9.1 (2026-08-30 — completed, not published), v0.9.2 (2026-08-31), v0.9.3 (2026-09-13 — merged, not published), v0.9.4 (2026-09-13 — merged, not published). Per-milestone phase detail, success criteria, and decisions for completed milestones live in `milestones/vX.Y-ROADMAP.md`. No milestone is active.*
