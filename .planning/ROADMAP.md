...
- [x] 44.1-01-PLAN.md — tracer: the resolved-level render gate recorded RED against the unfixed code, then BOTH parts of the repair (`visit_title` relative depth, `visit_toctree` context-relative offset) turn it green (SC#1, SC#2, SC#4, SC#5, D-07)

**Wave 2** *(parallel; disjoint test files, neither changes an emitted string)*

- [x] 44.1-02-PLAN.md — `tests/test_topics.py`: the four vacuous negative guards rewritten with a per-guard positive control, plus the clamp rationale twin (SC#6, SC#7 topic half, D-04/D-05)
- [x] 44.1-03-PLAN.md — the remaining five files: corrected grep-derived census, the toctree-side literal and its exactly-once regex, the regenerated byte-compared golden, and the empty/single-entry toctree edge (SC#6, D-04 addendum)
...
- [x] 44.2-02-PLAN.md — the precedence matrix (SC#2), multi-master no-leak for title AND author
      (SC#3), and D-03's four-route uniformity invariant

- [x] 44.2-03-PLAN.md — the corpus partition measured on both sides of the change (SC#4), the
...
- [x] 44.2-04-PLAN.md — gap closure for `44.2-VERIFICATION.md`'s one failed must-have: the published
...
- [x] 44.2-05-PLAN.md — gap closure for the re-verification's two failed truths (SC#2 and the
...
- [x] 44.2-07-PLAN.md — gap closure (round 4, wave 7) for `44.2-VERIFICATION.md`'s `missing:` bullet
...
- [x] 45.1-05-PLAN.md — DOC-13: publish the nine-parameter contract, rewrite the precedence model onto exclusivity, and build-verify all twelve documented examples (D-A/D-C/D-E/D-G/D-J) (wave 4)
...
- [x] 45.2-04-PLAN.md — Post-fix main-tree proof: SC#2 census + second-sync check, SC#1 tox matrix, SC#3 before/after census
...
- [x] 46-01-PLAN.md — Tracer: merge `origin/main`, repair the Windows contract-claims gate, and prove
      the CI loop live (D-20, D-22, D-23 run 1)

- [x] 46-02-PLAN.md — Version-literal lockstep bump and editable-install regeneration (SC#1)
- [x] 46-03-PLAN.md — Curated `## [0.7.1]` CHANGELOG entry, migration guide, and page-gate tuple (SC#2)
- [x] 46-04-PLAN.md — SC#3 green tree: the CI authority run (D-23 run 2) plus local docs, corpus and
      `ja` evidence
