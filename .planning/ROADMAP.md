...
- [x] 12-02-PLAN.md — XREF-01 cross-references: required `depart_term` `<label>`-anchor fix (fatal `:term:` bug) + `:ref:`/`:term:` render gate (wave 2)

**Wave 3** *(shares translator.py / test_pdf_render_gate.py with Wave 2)*

- [x] 12-03-PLAN.md — DESC-01…04 autodoc signatures: `desc_returns`/`desc_optional`/`desc_inline` + `desc_signature_line` `linebreak()` + render gate (wave 3)
...
- [x] 13-02-PLAN.md — BLK-03 `line`/`line_block` verbatim breaks + per-depth `h()` indent + line_block unit tests (wave 2)
...
- [x] 14-01-PLAN.md — FN-01 footnote handlers: `visit_document` pre-pass index (D-01) + `visit_footnote` SkipNode (D-05) + `visit_footnote_reference` definition/reuse/dangling (D-02/D-03/D-06/D-08) + `tests/test_footnotes.py` unit module (wave 1)
...
- [x] 15-02-PLAN.md — SC#3 empty-URL before/after machinery (git-worktree revert of XREF-01 `depart_term`, `-b typst` both sides)
