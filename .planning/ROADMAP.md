...
**No active milestone.** v0.9.2 shipped 2026-08-31 — `typsphinx 0.9.2` is on PyPI, tagged
`v0.9.2` at merge commit `45962faa`, with the GitHub Release body proven byte-identical to the
curated `## [0.9.2]` CHANGELOG section. It carries v0.9.1's completed-but-unpublished Windows path
work as well as its own inline-image blocker fix, under one heading; **no `## [0.9.1]` heading and
no `v0.9.1` tag were ever created**, because that release was cancelled rather than skipped.

Phase numbering is **continuous across milestones** — v0.9.2 ran Phases 62–63, so the next milestone
starts at **Phase 64**. Scope the next one with `/gsd-new-milestone`.
...
(never resets to 1). v0.9.2 ran Phases 62–63, so the next milestone starts at **Phase 64**.
...
7/7 v1 requirements complete; `verified_closeout`. An image not first in its container is now
separated before `image(` by the triad every other inline emitter already uses — a 9-line pure
insertion, gated by a real `typst.compile()` over 16 measured failing shapes and 9 that must keep
passing, recorded RED against the unfixed tree first. Full phase detail, the 14 binding constraints,
...
cancelled, not missed. It carried forward into v0.9.2 and closed there; this milestone's product
work reached users as part of **0.9.2**, under that heading, and no `v0.9.1` tag exists. Full phase
detail, the 14 binding constraints, success criteria and decisions:
...
their surrounding integers), with the prep-only Release phase last so its CHANGELOG entry describes
work already proven by the preceding phase's gate.

Phases 1–63 shipped or completed across v0.4.4 → v0.9.2; their per-phase plan counts, statuses and
completion dates are preserved in each milestone's archived roadmap under `milestones/`. This table
tracks the active milestone only, and there is none — the next entry is Phase 64.

## Roadmap Evolution

Per-milestone evolution notes are archived with their milestone. v0.9.2's — the four decisions baked
into the 62–63 split (fix and gate in one phase; research's Phase C not a phase; REL-10 folded into
release prep; the 3-OS matrix run deliberately given no REQ-ID), the inverted `gsd/v0.9.2-milestone`
decoy-branch correction, and the two PROJECT.md target-feature claims research falsified before the
roadmap was written — live in [milestones/v0.9.2-ROADMAP.md](milestones/v0.9.2-ROADMAP.md).
...
**Filed during v0.9.1** (2026-08-30), both in `translator.py`, neither in that milestone's
requirement scope. The first was promoted into v0.9.2 and is **closed**; the second is still open:

- `2026-08-29-inline-image-in-paragraph-emits-unseparated-expression` — `severity: blocker`.
  **Promoted into v0.9.2 as Phase 62 (IMG-08, IMG-09, IMG-10, TEST-05), fixed, and shipped in
  0.9.2**; the todo is in `todos/completed/`, closed against the full 16-shape matrix. An image
  node that is not the first
...
`2026-07-22-add-sphinx-linkcheck-ci-job` (Future LNK-01),
`2026-07-22-modernize-typing-imports-drop-up006-up035-ignore` (forbidden by `CLAUDE.md` until the
...
**Promoted into v0.9.2 and closed** (2026-08-31):

- `2026-08-29-inline-image-in-paragraph-emits-unseparated-expression` → Phase 62 (**IMG-08**,
  **IMG-09**, **IMG-10**, **TEST-05**). Shipped in 0.9.2.
- `2026-08-04-release-create-job-missing-uv-verify-end-to-end` (Future **REL-04**) → closed at the
  v0.9.2 close, not by a phase: `release.yml`'s `create-release` job completed `success` on the real
  `v0.9.2` tag push (run `33318905691`), its third consecutive green, and the published GitHub
  Release body was measured byte-identical to `scripts/extract_changelog_section.py 0.9.2`. This
  todo had been untestable at the v0.9.1 close because nothing was published.

**Known limitations still shipped with no published surface** after v0.9.2: WR-02's `confdir` gap
and the tripled "Custom template not found" warning, both carried unchanged since v0.9.0, plus
NUM-01's `numref` per-master divergence. The inline-image blocker is **no longer** among them — it
is fixed, published, and named in 0.9.2's release notes with an explicit "0.9.0 users should
upgrade". That is the **fifth consecutive** cycle at which a `### Known Limitations` section was
declined.

---
*Roadmap created: 2026-07-04 · Reorganized at each milestone close: v0.4.4 (2026-07-05), v0.5.0 (2026-07-11), v0.6.0 (2026-07-13), v0.6.1 (2026-07-19), v0.6.2 (2026-07-23), v0.6.3 (2026-07-25), v0.6.4 (2026-07-28), v0.6.5 (2026-07-29), v0.7.0 (2026-08-04), v0.7.1 (2026-08-11), v0.8.0 (2026-08-15), v0.9.0 (2026-08-22), v0.9.1 (2026-08-30 — completed, not published), v0.9.2 (2026-08-31). Per-milestone phase detail, success criteria, and decisions for completed milestones live in `milestones/vX.Y-ROADMAP.md`. No active milestone — scope the next with `/gsd-new-milestone`.*
