...
- **The bump mechanics.** `pyproject.toml:7` is the sole hand-edited version literal; `uv.lock` is
  ...
  - **AMENDED 2026-09-20 (owner-approved, raised by research, measured during planning).** This
    bullet's "the four files" and ROADMAP SC1's "the four files" name **different** four-item sets.
    This bullet lists `pyproject.toml`, `uv.lock`, `README.md` and
    `tests/test_changelog_page_gate.py`; `ROADMAP.md:467-469` (SC1) and the Phase Boundary bullet
    above (`:12`) list `pyproject.toml`, `uv.lock`, `README.md` and `CHANGELOG.md`. **Resolution:
    the bump commit carries the union — five files.** `git show --name-only` on it lists
    `pyproject.toml`, `uv.lock`, `README.md`, `CHANGELOG.md` and
    `tests/test_changelog_page_gate.py` together. The union is a strict superset that satisfies
    SC1's binding text literally and this bullet's "all of it lands in one commit" simultaneously;
    no other reading satisfies both. **Consequence for decomposition:** the entire `## [0.9.6]`
    curation (D-01 … D-12, the fresh `## [Unreleased]`, and the tail link block) lands in the SAME
    commit as the version bump — bump and CHANGELOG **cannot** be split across plans, waves or
    commits. Phase 46's two-commit split is explicitly NOT the precedent here.
