...
| SC3 | `### Known Limitations` in `## [0.9.6]` names NUM-01 only; the **anchored** `grep -c '^### Known Limitations$' CHANGELOG.md` goes **1 → 2** (the pre-existing `[0.1.0b1]` heading at `:1205` plus the new one). **Do not use the unanchored `grep -c 'Known Limitations'` form** — it already returns 2 today because `:665` is a prose mention inside a `[0.1.0b1]` bullet, so it would read 3 after this phase and a `== 2` check would fail. Corrected by the planner against a live measurement. | build + state | `75-CHANGELOG-EVIDENCE.md` |
