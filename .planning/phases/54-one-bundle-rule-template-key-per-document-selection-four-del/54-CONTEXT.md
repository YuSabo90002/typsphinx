...
- **D-15:** The relocation **announces itself** rather than changing behaviour silently: the same `config-inited` handler D-06 introduces also warns when `<srcdir>/base.typ` exists while `<srcdir>/_typst/base.typ` does not — naming the new location and stating the observable consequence (the build now typesets with the bundled default instead of that file). Without it a shadow project upgrading to v0.9.0 gets a silently different PDF and no diagnostic, which is the same failure shape SC#5 exists to prevent for the three removed config values. `app.srcdir` is available at `config-inited`; `app.builder` is not (D-10), so this warning appears in `-b html` builds too, on the same terms already recorded in D-10. Severity is `logger.warning`, build continues — D-07's reasoning applies unchanged. — **Reversibility:** reversible — a warning with no caller-visible contract.
