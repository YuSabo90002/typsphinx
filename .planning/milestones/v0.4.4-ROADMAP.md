...
**Phases:** 1–5
**Total Plans:** 15
**Git range:** origin/main @ PR #106 (merge 733067f) → tag `v0.4.4`

> Archived from `.planning/ROADMAP.md` at milestone close. The active roadmap now
> carries only the collapsed milestone summary. Full phase detail is preserved below.
...
**Decimal Phases:** None — all five phases were planned integer phases; no urgent insertions were needed.

**Key Decisions:**

- Pin runtime deps to known-good rather than forward-port to sphinx 9 / typst 0.15 — fastest, lowest-risk path to green in a maintenance cycle (the `kai` break came from a bundled `@preview` package under typst 0.15).
- `typst>=0.14.1,<0.15` empirically confirmed in CI as the compatible line for the bundled `@preview` packages; sphinx/docutils ceilings recorded as precautionary vs load-bearing per PIN-06.
- Modernize Python floor to 3.10–3.13 as one atomic, CI-verified batch (Phase 3), sequenced only after the pin fix was confirmed green.
- Conservative dev-tooling bumps with guard ceilings (stay on `pytest~=8.4`, `mypy<2.0` this cycle); artifact actions bumped to node24 ahead of GitHub's 2026-09-16 Node-20 removal.
- Durability guardrails: `uv sync --locked` at all 9 sites (DUR-01), standalone weekly + dispatch `drift.yml` forward-drift detector (DUR-02), scoped `sphinx-typst-stack` Dependabot group (DUR-03), README CI badge (DUR-04).

**Issues Resolved:**

- CI fully red (lint + 12-lane matrix + coverage + docs PDF) → green across the 3-OS × Python matrix (19 jobs) plus `docs.yml` end-to-end incl. the multi-language PDF-copy step.
- tox env-name mismatch (`py3.10` vs `py310`) that failed 9/12 matrix jobs — fixed via matrix→tox env mapping (Phase 2).
- Python-floor bump ruff/pyupgrade regressions + `tomllib`-on-3.10 docs break — remediated in-batch on PR #104 (Phase 3).
- Stale `Test Python 3.9` required check removed from main branch protection (Phase 4).

**Issues Deferred:**

- D-11: `softprops/action-gh-release@v3` tag-gated runtime confirmation — signed-off deferral to the next real release tag (this v0.4.4 release is that tag). Not an open requirement.
- v2 forward-ecosystem support (FWD-01/02/03: Sphinx 9, typst 0.15+, configurable `@preview` versions) — explicitly out of scope for this maintenance cycle.

**Technical Debt Incurred:**

- The 3-way hardcoded `@preview` version sync (`writer.py`, `template_engine.py`, `templates/base.typ`) remains, now guarded by an automated desync test (Phase 2) rather than eliminated. Full removal tracked as FWD-03.
...
---
*Roadmap created: 2026-07-04 · Archived: 2026-07-05 at v0.4.4 milestone close*
