# Requirements Archive: v0.7.0 API rendering design overhaul
...
- [ ] **REL-04** [M]: The GitHub Release body is the **curated `## [X.Y.Z]` CHANGELOG section**, not
  ...
  **NOT MET at the v0.7.0 close (2026-08-04) — carried to v0.7.1.** The workflow change landed in
  Phase 41 (plan 41-01) and is correct in isolation, but its first real tag push failed: the
  `create-release` job runs `uv run python scripts/extract_changelog_section.py` and that job has no
  `astral-sh/setup-uv` step — `validate` and `build` both do, `create-release` never needed uv before
  REL-04 wired the extractor into it. Release run `30848860064` therefore went
  `validate` ✓ → `build` ✓ → `publish-pypi` ✓ → `create-release` ✗ (`uv: command not found`,
  exit 127). PyPI published fine; the GitHub Release was left as the empty-bodied, PDF-only entry
  `docs.yml` creates. The v0.7.0 release body was repaired **by hand** at the close (curated
  `## [0.7.0]` section + Installation block + GitHub's auto-generated notes, plus the wheel and
  sdist uploaded), so the *published artifact* now matches what REL-04 describes — but the
  *automation* has still never produced it, which is what this requirement asserts. `release.yml`'s
  `create-release` job gained the missing `Install uv` / `Set up Python` steps on `main` after the
  release; the requirement closes only when a real tag push exercises it end to end.

- [x] **REL-05** [M]: v0.7.0 is released — version bumped as the sole literal in `pyproject.toml`
...
| REL-04 | Phase 41 | Pending — carried to v0.7.1 (see note below) |
| REL-05 | Phase 41 | Complete |
