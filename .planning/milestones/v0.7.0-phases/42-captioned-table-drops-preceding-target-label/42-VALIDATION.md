...
|---------|------|------|-------------|------------|-----------------|-----------|-------------------|-------------|--------|
| 42-01 T1/T2/T3 | 42-01 | 1 | TBL-03 (SC#1/SC#5) | T-42-01, T-42-02 | N/A | classic real-compile RED (`TypstError` / `does not exist in the document`) recorded against the unfixed `depart_table`, for all four D-01 shapes | `uv run pytest tests/test_captioned_table_propagated_target_render_gate.py -v` (must exit NON-zero pre-fix) | ❌ W0 — new fixture + module, created by 42-01 T1/T2 | ⬜ pending |
| 42-04 T1 | 42-04 | 2 | TBL-03 (SC#3) | T-42-01, T-42-02, T-42-10 | N/A | same module re-run to GREEN after the `_emit_id_anchors` call-ordering move; both labels resolve, no duplicate-label fatal | `uv run pytest tests/test_captioned_table_propagated_target_render_gate.py -v` | ✅ exists after 42-01 | ⬜ pending |
| 42-02 T1/T2/T3 | 42-02 | 1 | TBL-03 (SC#2 / D-09, D-10) | T-42-04, T-42-06 | N/A | permanent figure regression gate — real compile over D-10's three figure shapes | `uv run pytest tests/test_figure_propagated_target_render_gate.py -v` | ❌ W0 — new fixture + module (NOT an extension of `figure_target_caption_render_gate/`), created by 42-02 T1/T2 | ⬜ pending |
| 42-05 T1/T2 | 42-05 | 3 | TBL-03 (SC#4 / D-04) | T-42-11, T-42-12 | N/A | two-worktree byte-diff over the caption-less path, recorded as evidence (not a pytest assertion) | `diff <pre-fix-build>/index.typ <post-fix-build>/index.typ` must be empty, exit 0 — see `42-RESEARCH.md` § 4 | N/A — evidence-file method | ⬜ pending |
| 42-03 T1/T2/T3 | 42-03 | 1 | TBL-03 (D-06 / D-07 / D-08) | T-42-07, T-42-08, T-42-09 | N/A | repo-wide sweep for the same misrouting class, recorded as evidence including a null result; plus the D-08 todo | `grep -n "_emit_id_anchors(" typsphinx/translator.py` + classification per `42-RESEARCH.md` § 5, and a real `-b typstpdf` image-path probe build | N/A — evidence-file method | ⬜ pending |
| 42-06 T1/T2/T3 | 42-06 | 3 | TBL-03 (SC#6) | T-42-14, T-42-15 | N/A | Phase 41 reconciliation — CHANGELOG TBL-03 line + SC#4 invariant sweep re-measured over a SHA range including Phase 42 + REL-04/REL-05 flip guard | see `42-RESEARCH.md` § 6; CHANGELOG placement checked by the `awk` gate in 42-06 T1 | N/A — docs/evidence | ⬜ pending |
