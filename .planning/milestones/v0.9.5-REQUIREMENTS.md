# Requirements Archive: v0.9.5 Docs Link Check and Navigation
...
- REL-14 is mapped to Phase 73 for coverage only; its checkbox is checked at
  `/gsd-complete-milestone`, on the observed merge. **Checked 2026-09-16T12:06:03Z** on the five
  required observations: PR #151 merged as `43fd7c13` on `origin/main`'s first-parent history;
  `origin/main:pyproject.toml:7` reads `version = "0.9.2"`; `git ls-remote --tags origin` shows
  `refs/tags/v0.9.2` (positive control) and no `v0.9.3`/`v0.9.4`/`v0.9.5`; PyPI returns `200` for
  `0.9.2` and `404` for `0.9.3`, `0.9.4` and `0.9.5`; `gh release list` shows `v0.9.2` as Latest
  and no `v0.9.3`/`v0.9.4`/`v0.9.5` Release.

---
*Requirements defined: 2026-09-13*
*Last updated: 2026-09-16 at `/gsd-complete-milestone` — REL-14 checked on the observed merge (PR #151 → `43fd7c13`); 4/4 v1 requirements complete*
