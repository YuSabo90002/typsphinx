# Requirements Archive: v0.9.0 per-document templates

**Archived:** 2026-08-22
**Status:** SHIPPED

This is the complete, final requirements record for v0.9.0. `.planning/REQUIREMENTS.md` was removed
at this close; `/gsd-new-milestone` creates a fresh one for the next milestone.
...
- [x] **REL-08**: v0.9.0 is published — PyPI wheel + sdist, GitHub Release carrying the curated
      ...
      *Closed at `/gsd-complete-milestone` on 2026-08-22, on the publish. PR #134 merged to `main`
      (`68b92e24`) with all 15 checks green; tag `v0.9.0` pushed on the merge commit; release run
      `32560457509` completed `validate` → `build` → `publish-pypi` → `create-release` all success —
      including `create-release`, the job that failed at the v0.7.0 close. PyPI `typsphinx 0.9.0`
      live (wheel 187,400 B + sdist 809,731 B); the GitHub Release body is byte-identical to
      `scripts/extract_changelog_section.py 0.9.0`'s 123-line stdout, carrying all three assets;
      `typsphinx-doc-translations` advanced its pin to `68b92e24` via its own dispatched
      `update-pin.yml` (run `32566262655`) and was tagged `v0.9.0` on `f9390d4b`. The
      `phase.complete` auto-flip fired for the fifth consecutive release-prep close during Phase 57
      and was caught and reverted there; this flip is the intended one.*
...
| REL-08 | Phase 57 + `/gsd-complete-milestone` publish | Complete |
