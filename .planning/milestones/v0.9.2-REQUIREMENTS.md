# Requirements Archive: v0.9.2 Inline image blocker fix and release
...
- [x] **REL-09**: 0.9.2 released to PyPI with a curated `## [0.9.2]` CHANGELOG entry, the version
...
- [x] **REL-10**: The "Planned for Future Releases" scratch block currently nested under
...
- [x] **REL-11**: The release requirement's checkbox is protected by a SHA-256 of
...
| REL-09 | Phase 63 | Complete |
| REL-10 | Phase 63 | Complete |
| REL-11 | Phase 63 | Complete |
...
## Release outcome (recorded at `/gsd-complete-milestone`, 2026-08-31)

REL-09, REL-10 and REL-11 were held at `[ ]` through every plan of Phase 63 and are checked here,
at the close, on observed evidence — not by `phase.complete`.

- **REL-11 first, because it licenses the other two.** The `63-CLOSEOUT-GUARD.md` Baseline was
  re-verified once more at milestone close, after all `phase.complete`-family tooling had run:
  `sha256sum` `f0dd4ec377bbc95cd2b8cdb19fe784cfc21bd6d08e2743de6f5b9fc1768f5b33`, `wc -l` `184`,
  `git diff --name-only -- .planning/REQUIREMENTS.md` empty, and the three `REL-09` hits still at
  lines 70/154/175 — **MATCH on all four**. That is the third and decisive observation the guard
  file owed. The flip that has landed at five consecutive prior release-prep closes did not land
  here; the edit above is the operator's, taken after the publish.

- **REL-09.** `typsphinx 0.9.2` is on PyPI (`typsphinx-0.9.2-py3-none-any.whl` and
  `typsphinx-0.9.2.tar.gz`; `info.version == "0.9.2"`), published by `release.yml` run
  `33318905691` from tag `v0.9.2` on merge commit `45962faa`. All jobs `success`:
  `Validate Release` (which runs `scripts/extract_changelog_section.py 0.9.2` as a real
  precondition), `Build Distribution`, `Publish to PyPI` (after the `pypi` Environment's manual
  approval), `Create GitHub Release`. GitHub Release `v0.9.2` exists, `isDraft: false`.

- **REL-10.** Closed inside Phase 63 by running the extractor and reading its stdout
  (`63-CHANGELOG-EVIDENCE.md`), and re-confirmed here by `Validate Release` passing on the
  published tag: the `## [Unreleased]` heading is present and empty of release content above
  `## [0.9.2]`, and the `### Planned for Future Releases` scratch block sits under it, not in the
  extracted body.

**REL-04 observed green, and its todo closes.** `Create GitHub Release` — the job that failed with
`uv: command not found` at the v0.7.0 tag push — completed `success` on this real tag push, its
third consecutive green after v0.8.0 and v0.9.0. Read from the job's own conclusion, not inferred
from the run status. `63-HANDOFF.md` § 2 (c) said to fix it inside this release work if it failed;
it did not fail.

*Requirements defined: 2026-08-30*
*Last updated: 2026-08-31 at v0.9.2 milestone close — REL-09/REL-10/REL-11 checked on the observed
publish; 7/7 v1 requirements complete*
