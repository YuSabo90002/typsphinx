...
## Milestone: v0.9.3 — Toolchain and dependency-update repair

**Completed:** 2026-09-13 · **Not published**. Merged to `main` via PR #143 with no tag, no PyPI upload and no GitHub Release
**Phases:** 6 (64–69) | **Plans:** 27 | **Tasks:** 70

### What Was Built

- **FHS command shims in `flake.nix`** (NIX-01..NIX-08, Phase 64). Seven bare commands on PATH, each resolving the checkout's own `.venv/bin/<tool>` inside a Linux-guarded `buildFHSEnv`, failing loudly (exit 127) when the tool is missing. `ruff`, every tox environment, `.venv/bin/uv` and tox's downloaded CPython now execute on the maintainer's NixOS machine. The manual per-worktree `ln -sf` / `patchelf` step is retired.
- **`tox-uv-bare` → `tox-uv`** (TOX-01..TOX-04, Phase 65). A four-file drop-in swap with `uv.lock` regenerated. A live tox run showed the bundled branch resolves the same `.venv/bin/uv` the shim does.
- **Dependabot on the `uv` ecosystem** (DEP-01..DEP-05, Phases 66–67). The switch landed on `main` through its own PR (#137). The proof is dependabot's own #138, which updated `pyproject.toml` and `uv.lock` together and ran the whole CI. #123 and #128 were closed on their merits.
- **Documentation of the landed mechanism** (DOC-19..DOC-21, Phase 68). The `CLAUDE.md` NixOS subsection and worktree boundary, the `tox.ini` pin rationale, and the `flake.nix` structure notes. Each was proven behaviour-neutral by a hash rather than asserted.

### What Worked

- **Reading the source before trusting the roadmap's mechanism claim.** Constraint 12 said tox-uv's bundled `uv` lookup was "a different code path" from `uv.find_uv_bin()`. Phase 65's discussion read `tox_uv/_venv.py:232-238` and a scratch probe: it is the same function. The claim was amended with the owner before any plan was written. This is the third consecutive milestone to falsify a locked statement by measurement.
- **Following the tool's real constraints instead of the plan's shape.** "One PR to `main`" could not survive dependabot reading its config only from the default branch. A separate `dependabot.yml`-only PR, byte-identical to the milestone branch's copy, kept REL-12's merge conflict-free. The trial merge at close was clean for exactly that reason.
- **Two tracks that shared nothing, stated up front.** The NixOS track and the dependabot track touched no common file, and the roadmap said so. Neither track's failures blocked the other.
- **The checksum fence, for the eighth time.** `phase.complete` flipped REL-12 at Phase 69's close as it has at every release-prep close. The `cp` backup → revert → SHA-256 re-match procedure caught it mechanically, and the box was checked only after the PR merge was observed.
- **A real audit before the close.** `v0.9.3-MILESTONE-AUDIT.md` classified REL-12 as held by design and listed 16 debt items. It also let the owner fix two CHANGELOG wording defects before the PR rather than after.

### What Was Inefficient

- **The first shim re-measurement found a missing `libz.so.1`.** uv-managed interpreters needed `zlib` in the FHS closure, and the first acceptance pass failed 3 of 7 gates. Closing it took a gap-closure plan plus a full Claude Code relaunch, because the shims reach a session only through the launching shell's PATH.
- **All six verifications went fingerprint-stale by the close.** Every VERIFICATION.md lists `.planning/REQUIREMENTS.md` in `covered_files`, and later phases legitimately edit that file. The readiness check therefore could not pass without re-running `/gsd-verify-work` six times, and the close became an `override_closeout` for a tooling reason rather than a quality one.
- **The grouped-update path is still unproven.** Sphinx 9.1.0's `docutils<0.23` cap prevents any `sphinx-typst-stack` group PR from opening under `uv`, so the one dependabot behaviour most likely to differ between ecosystems went unobserved.
- **Nyquist and security gates did not run in any phase**, though both are enabled in config.

### Patterns Established

- **Put a config change the tool reads only from the default branch on `main` early, through its own PR, with a byte-identical copy on the milestone branch.** The milestone PR then merges clean.
- **A shim that cannot find its binary must fail, never fall back.** "Runs but lies" (some other `ruff`) is worse than "doesn't run".
- **Measure the environment the executor actually gets.** The shims exist only in a session launched from the direnv-loaded shell. Re-measurement after a fix happened in a relaunched session, not in the one that wrote the fix.

### Key Lessons

1. A mechanism statement in a roadmap is a claim about someone else's code. Reading that code (`_venv.py:232-238`) cost minutes and changed the plan.
2. A covered-files fingerprint that includes a shared tracking file makes every earlier verification stale by construction. Either the fingerprint should exclude tracking files, or the close should expect the override.
3. The auto-flip hazard is now eight-for-eight on release-prep closes and the fence eight-for-eight on catching it. Treat it as a permanent property of the tooling.
4. A milestone that publishes nothing still has one irreversible step, the merge. Naming it as a requirement (REL-12), with observations defined before the close, made it as checkable as a publish.

### Cost Observations

- Model mix: adaptive profile (`model_profile: adaptive`): opus for discussion/planning/verification, sonnet for execution.
- Sessions: 12 calendar days (2026-09-02 → 09-13), 238 branch commits before the close; worktree-isolated executor mode throughout, now running through the shims it built.
- Notable: code delta outside `.planning/` is 10 files, +329 / −91, with `typsphinx/` untouched. Every CI dispatch in the milestone (64, 65, 69) was green on the first attempt.
---

## Milestone: v0.9.4 — Typing Modernization

**Completed:** 2026-09-13 · **Not published**. Merged to `main` via PR #145 with no tag, no PyPI upload and no GitHub Release
**Phases:** 2 (70–71) | **Plans:** 20 | **Tasks:** 42

### What Was Built

- **The `UP006`/`UP035` ignores are gone and the style is enforced** (QUA-09, QUA-11, Phase 70). 113 findings across 10 files, measured fresh rather than taken from the stale todo, converted by scoped two-pass `ruff --fix` with one hand edit, then the ignores dropped in one commit together with the todo move.
- **Five-leg behaviour-identity evidence** (QUA-12). Masked-AST hashes, pytest counts, zero assertion edits, a byte-identical 167-project `.typ` corpus, and identical `mypy` output, all MET.
- **The visible change traced** (DOC-23). 83 docs hunks of API-reference type text, each mapped to a converted source line and read by the owner at UAT.
- **`CLAUDE.md` rewritten ahead of the conversions** (DOC-22), and **close prep plus the merge** (REL-13): one CHANGELOG bullet, green tree locally and in CI, PR #145 merged and observed.

### What Worked

- **Piloting the evidence harness before automating it.** The masked-AST harness ran by hand on two files, one of each shape (shrink-in-place and move-and-vanish), with two mutation controls that must change the hash. The later waves then ran it on eight more files without a single false EQUAL.
- **Measuring the census instead of trusting the todo.** The 2026-07-22 todo named four `typsphinx/` files and no tests; the fresh census found ten files. Discovery by `ruff check . --select UP006,UP035` made the stale list irrelevant.
- **Ordering the documentation change first.** `CLAUDE.md` was worded to be true before and after the flip, so no worktree executor ever read an instruction that contradicted the tree it was editing.
- **The checksum fence, even when nothing fired.** `phase.complete 71` left REL-13 alone for the first time in nine release-prep closes. The fence still ran and still matched; one non-firing was not treated as a fix.
- **A single-day milestone.** Two phases, 20 plans, 144 commits, and the merge on the same calendar day, because the scope was one mechanical rewrite plus its evidence.

### What Was Inefficient

- **A GitHub HTTP 5xx created a silent duplicate CI run.** The dispatch API returned 500/502, one "failed" call had created a run, and a retry created a second. The plan's "exactly one dispatch" check failed literally and needed an owner decision (AMENDED reading, history kept).
- **Phase 70 went fingerprint-stale by construction again.** A later phase's legitimate edit to `REQUIREMENTS.md` (REL-13's AMENDED block) made the readiness check fail, and the close became an `override_closeout` for a tooling reason for the second milestone running.
- **A plan oracle broke on locale.** 70-12's automated verify grepped English text out of `diff -rq` output that the sandbox printed in Japanese. The evidence itself was sound; the oracle was not.
- **Phase 71 skipped Nyquist validation and the security audit**, though both are enabled.

### Patterns Established

- **For a mechanical refactor, write the behaviour-identity evidence as independent legs**, each covering a failure mode the others cannot see, and pilot any harness against controls that must fail before trusting its EQUAL.
- **Before retrying a GitHub API call that returned 5xx, list what exists.** The object may already have been created server-side.
- **Word a standing instruction to be true across the change it precedes**, so the order of documentation and code no longer matters to anyone reading mid-milestone.

### Key Lessons

1. A todo's file list is a snapshot of its author's search, not of the tree. Re-derive discovery with the tool that will enforce the rule.
2. "The tooling stopped doing the wrong thing once" is one observation. Keep the guard until the cause is understood.
3. HTTP 5xx is not evidence of absence. Check before retrying any call that creates something.
4. A covered-files fingerprint that includes a shared tracking file will keep turning earlier phases stale. Two closes in a row now confirm it.

### Cost Observations

- Model mix: adaptive profile (`model_profile: adaptive`): opus for discussion/planning/verification, sonnet for execution.
- Sessions: 1 calendar day (2026-09-13), 144 branch commits before the close; worktree-isolated executor mode throughout.
- Notable: code delta outside `.planning/` is 13 files, +90 / −84, with `typsphinx/` at +65 / −65 in annotations and imports only. Both CI dispatches (70, 71) and PR #145's 15 checks were green on the first attempt.
## Milestone: v0.9.5 — Docs Link Check and Navigation

**Completed:** 2026-09-16 · **Not published**. Merged to `main` via PR #151 with no tag, no PyPI upload and no GitHub Release
**Phases:** 2 (72–73) | **Plans:** 13 | **Tasks:** 31

### What Was Built

- **`tox -e linkcheck`** (QUA-13, Phase 72). A new `[testenv:linkcheck]` shaped exactly like the existing `docs-html` environment and kept outside `env_list`, so plain `tox` never reaches the network. 95/95 links `working` on a clean run, exit 0, with no `linkcheck_*` key added to `conf.py`.
- **Listed on every surface that lists the environments** (DOC-24). The three surfaces came from a fresh repo-wide grep — `CLAUDE.md`, `README.md`, `docs/source/contributing.rst` — not from the requirement's own wording.
- **The sidebar matches the hierarchy** (DOC-18). The five duplicated section children are gone from the root toctrees: `multiple toctrees` messages 5 → 0, both docs warning counts unchanged, each page still included exactly once under `-b typst`.
- **Close prep plus the merge** (REL-14, Phase 73 + the close). Two CHANGELOG subsections as a pure addition under `## [Unreleased]`, green tree locally and in CI, and PR #151 merged and observed on five positively-controlled probes.

### What Worked

- **Deferring QUA-08 at roadmap review instead of shipping an unexercised workflow file.** GitHub runs `schedule` only on the default branch and `workflow_dispatch` only for a file already there, so a new workflow could not have been proven on the unmerged milestone branch. The milestone shipped only what it could measure, and left the obstacle documented.
- **Deriving the DOC-24 surface list from a fresh grep.** The requirement named "wherever the tox environments are listed"; the grep found three surfaces, one of which (`docs/source/contributing.rst`) no earlier note had mentioned.
- **Re-measuring the todo's own numbers.** The 2026-08-16 todo recorded 4 `multiple toctrees` messages; 5 were measured, because `examples/advanced` had since been added. Planning used the measurement.
- **Disposing of the narrower divergence inside the phase that raised it.** The HTML-vs-Typst `examples/basic` parent question was re-measured at the phase tip, found not to survive (`DIVERGENCE_SURVIVES = no`), and closed without filing a todo.
- **The handoff as an executable step list.** `73-HANDOFF.md` told the close exactly what to run and in what order, including the conditional branch update — and the condition it predicted (`main` has moved) is the branch that actually fired.
- **The checksum fence, which had something to catch this time.** `phase.complete 73` flipped REL-14 against the CONTEXT decision; the fence caught it and it was reverted before commit.

### What Was Inefficient

- **Both phases went fingerprint-stale, not just the earlier one.** For the first time the whole milestone read `phase_complete: false` at the readiness check, so the close was an `override_closeout` for a tooling reason for the third milestone running. Phase 73 went stale from its own post-verification tracking commits, which no ordering can avoid.
- **The planning premise about Dependabot was overtaken by events.** Phase 73 was planned on 2026-09-14 around ordering #146–#150 after the milestone PR; all five had already merged that same day, and the phase had to re-measure live and rewrite its own handoff section.
- **Phase 73 skipped Nyquist validation and the security audit**, though both are enabled — the same omission as v0.9.4's Phase 71, in the same kind of prep-only phase.
- **72-VERIFICATION.md's body still says `human_needed` while its frontmatter says `passed`.** The frontmatter is correct (72-UAT.md closed the one human item), but the stale body line is the same shape v0.9.4's 70-VERIFICATION.md carried.

### Patterns Established

- **A workflow file that cannot run from the branch that introduces it is not provable in that milestone.** Decide at scoping whether to plan a side PR to the default branch or defer; do not discover it at execution.
- **Write the close handoff as an ordered step list with its conditional branches named**, each citing the evidence file and key that justifies it, so the close runs measurements rather than re-deriving verdicts.
- **A prep-only phase's own tracking commits will stale its verification.** Treat `override_closeout` on the final phase as expected, and check the VERIFICATION.md verdict rather than the readiness projection.

### Key Lessons

1. Scope a CI workflow against where GitHub will actually run it, not against where the work happens.
2. A phase planned days before it executes must re-measure its external premises; the plan's prose is not authoritative about the outside world.
3. "Nine of ten" is still the rule. The one close where the auto-flip did not fire stayed an outlier, and the fence caught it again.
4. The covered-files fingerprint now stales *every* phase in a two-phase milestone. Three closes in a row confirm it; the readiness projection is no longer a usable gate here.

### Cost Observations

- Model mix: adaptive profile (`model_profile: adaptive`): opus for discussion/planning/verification, sonnet for execution.
- Sessions: 3 calendar days (2026-09-13 → 2026-09-16), 101 branch commits before the close; worktree-isolated executor mode throughout.
- Notable: code delta outside `.planning/` is 6 files, +26 / −5, with `typsphinx/` and `.github/workflows/` untouched for the whole milestone. Both CI dispatches (72, 73) and PR #151's 15 checks were green on the first attempt.

---

## Milestone: v0.9.6 — Doctest block rendering and release

**Shipped:** 2026-09-22 · **PUBLISHED**. `typsphinx 0.9.6` on PyPI, tag `v0.9.6` on merge commit `6fcc5adb` via PR #156, GitHub Release created by `release.yml` run `35730551619`
**Phases:** 2 (74–75) | **Plans:** 14 | **Tasks:** 36 | **Quick tasks:** 2 (both at the close)

### What Was Built

- **A `doctest_block` handler** (TRN-01, Phase 74). `visit_literal_block` / `depart_literal_block` now accept `nodes.doctest_block` and supply the language tag themselves, because `HighlightLanguageTransform` sets `node['language']` only on `literal_block`. Before this, the node fell through `unknown_visit()` while its `Text` children were still re-emitted as inline text, so an `Examples:` docstring collapsed onto one line — and beside other content in the same container the emission was malformed enough that Typst refused the file outright.
- **A two-context real-compile gate** (TRN-02). Eleven tests over three masters, recorded observably RED against the pre-handler translator: the plain-paragraph doctest block is a genuine `typst.compile()` refusal, and the definition-list / bullet-list shapes collapse without a separator.
- **Zero-warning docs build** (QUA-14). The docutils `Unexpected indentation` and `Block quote ends without a blank line` messages this project's own docstrings raised are gone: `sphinx-build -b typst docs/source` goes `build succeeded, 5 warnings.` → `build succeeded.`, with neither message class allowed to rise while the other falls.
- **The release** (REL-15, REL-16, Phase 75 + the close). One five-file commit bumps `0.9.2` → `0.9.6` and curates the whole `## [0.9.6]` section, carrying v0.9.3's, v0.9.4's and v0.9.5's bullets with it. REL-16 ends a five-milestone run of declining a `### Known Limitations` section.
- **Two audit todos closed at the close itself**, as quick tasks rather than deferrals: IN-01's docstring `Args:` lines, and the PyPI `#history` linkcheck anchor.

### What Worked

- **Re-checking a deferral's *reason*, not just its status.** Both todos had been held back by Phase 75's prep-only fence, whose cost argument was that touching `typsphinx/` or `conf.py` forces SC4's whole green proof to be re-taken on a changed tip. By close time `origin/main` had moved (Dependabot #152–#155) and `main`'s protection is `strict`, so that re-take was mandatory anyway. The premise was gone, and both closed for no additional CI cost.
- **Reading the dependency's source instead of applying the todo's proposed fix.** The PyPI-anchor todo asked for a `linkcheck_anchors_ignore_for_url` entry covering the `#history` URL. Sphinx's `_check_uri` partitions the URI on `#` **before** matching those patterns, so that pattern could never match — a silent no-op that reads correctly in review. A bare `pypi\.org` entry would have prefix-matched every PyPI project page. Both were caught at planning time by reading `sphinx/builders/linkcheck.py`.
- **A differential oracle instead of a pass/fail one.** The linkcheck fix was proved by diffing per-URL status sets across a before-run and an after-run, with the two Class A tag records *expected* to stay broken. A plan asserting an unconditionally clean run would have failed for the wrong reason — and the deliberate choice not to write raw set equality kept third-party flakiness from failing it for a third wrong reason.
- **Bounding the suppression by measurement.** The landed pattern was compiled out of the module namespace the way Sphinx binds it, then matched against every URI in the before-run, asserting the suppressed set was exactly one anchor out of 41. The regex was never argued from, only measured.
- **The prediction in `75-HANDOFF.md` held exactly.** Its Class A obligation said the two `v0.9.6` tag URLs would resolve the moment the tag existed, and that a failure after that should be treated as a real regression rather than re-amended. They resolved; the result is this project's first unconditionally clean `tox -e linkcheck` — **96/96 `working`, exit 0**.
- **REL-15 checked by hand, last, against its own four named observations.** The auto-flip fired again at Phase 75's `phase.complete` (10 of 11 release-prep closes) and was reverted line-scoped so REL-16's legitimate close survived.
- **The planner correcting the orchestrator's own line numbers.** `260922-tbe`'s plan re-grepped rather than trusting the `2443-2444` figure it was handed, and found `2445`.

### What Was Inefficient

- **A fourth consecutive `override_closeout` for the same tooling reason.** Both phases read `verification_status: stale` while both VERIFICATION.md files read `passed`. The cause has been identical at every close since v0.9.3: `.planning/` tracking commits land after verification and touch files its fingerprint covers. This is now a per-close tax, not a finding.
- **MSG-06 was deferred a second time while its two siblings were closed.** All three were prep-only-fence deferrals with the same lapsed premise; two were picked up at the close and one was not, for no recorded reason.
- **QUA-08's obstacle has been gone for two consecutive milestones** without it being scoped. v0.9.5's retrospective already recorded the obstacle as removed.
- **Phase 75 skipped `validate-phase` again** — the third consecutive release-prep phase to leave `VALIDATION.md` at `status: draft` while `workflow.nyquist_validation` is on.
- **The archive tooling needed the same three hand repairs as last time.** MILESTONES.md's accomplishments came out as a raw dump of all 14 plan one-liners, several of them machine-readable `KEY = value` strings; STATE.md kept `current_phase: 75` after the close and flattened Current Position; ROADMAP.md needed its Backlog extracted and re-appended by hand.
- **A stale CDN read almost got mistaken for a failed build.** Read the Docs' `ja` page served `typsphinx 0.9.2` after the build had finished successfully at the new commit; only a cache-busted fetch showed `0.9.6`.

### Patterns Established

- **A deferral records a reason, and the reason can expire before the deferral is revisited.** At each close, re-check the *premise* of every carried deferral, not just its status — a cost that has already been paid for other reasons is no longer a cost.
- **When a todo proposes a concrete fix, verify it against the dependency's source before applying it.** A no-op that reads plausibly is worse than no fix, because it closes the tracking record.
- **Prove a suppression's scope by running it over the real input set**, not by reading the pattern. Print the sensitivity control too, so a probe that cannot discriminate says so.
- **A differential oracle should forbid the specific transition that indicates the defect**, not assert global equality — otherwise external flakiness fails it for the wrong reason.

### Key Lessons

1. Re-examine why something was deferred, not just that it was. Two of three prep-only deferrals closed at this milestone for free because the fence's own cost argument had lapsed.
2. A tracking todo's "Solution" section is a hypothesis. This one would have produced a silent no-op; ten minutes in `sphinx/builders/linkcheck.py` was the difference.
3. Four consecutive closes have been overridden for the identical fingerprint-staleness reason. The readiness projection is not a gate here and has not been for a year of milestones; the VERIFICATION.md verdict is.
4. "Ten of eleven" now. The REL auto-flip fired again, the fence caught it again, and line-scoping it let REL-16's legitimate close survive in the same file.
5. A green build and a green page are different observations. Cache-bust before reading a published page as evidence.

### Cost Observations

- Model mix: adaptive profile (`model_profile: adaptive`): opus for discussion/planning/verification, sonnet for execution.
- Sessions: 7 calendar days (2026-09-16 → 2026-09-22), 142 commits from the milestone base; worktree-isolated executor mode throughout, including both close-time quick tasks.
- Notable: the two close-time quick tasks cost no extra CI, because the dispatch they would have forced was already mandatory for an unrelated reason.
...
| v0.9.3 | ~12 days | 6 (no insertions) | First toolchain-only milestone, and the second to publish nothing — but the first to name its one irreversible step, the merge to `main`, as a requirement (REL-12) with its closure observations written down before the close. Two tracks that shared no file ran side by side: FHS command shims in `flake.nix` plus the `tox-uv` revert, and dependabot's switch to the `uv` ecosystem, proven on dependabot's own PR #138. A roadmap mechanism claim (tox-uv's "different code path") was falsified against the installed source before planning — the third consecutive milestone to amend a locked statement by measurement. A config the tool reads only from the default branch forced a mid-milestone PR to `main` (#137), byte-identical on the branch so the final merge stayed clean. `phase.complete`'s release-requirement auto-flip reached eight-for-eight and the checksum fence caught it again. `override_closeout` for a tooling reason: every VERIFICATION.md fingerprints `REQUIREMENTS.md`, so earlier phases go stale by construction, and a real `MILESTONE-AUDIT.md` stood in |
| v0.9.4 | **1 day** | 2 (no insertions) | First milestone to change `typsphinx/` while promising — and measuring — that nothing a user runs would change. A lint deferral held since the v0.6.2 close was retired by mechanical rewrite, and the rewrite itself was the easy part: the milestone's weight was a five-leg behaviour-identity proof (masked-AST hashes, pytest counts, zero assertion edits, a byte-identical 167-project `.typ` corpus, identical `mypy` output), with the hash harness piloted against must-fail controls before it was automated. The documentation change was ordered first and worded true on both sides of the flip. Third milestone to publish nothing and second to name the merge as its one irreversible step (REL-13). `phase.complete` **did not** flip the release checkbox for the first time in nine release-prep closes; the fence ran anyway. `override_closeout` for the same tooling reason as v0.9.3 (a fingerprinted `REQUIREMENTS.md`). A GitHub HTTP 5xx silently created a duplicate CI run, settled by an owner-approved AMENDED reading |
| v0.9.5 | ~3 days | 2 (no insertions) | First milestone aimed entirely at the project's own documentation surface, with `typsphinx/` and `.github/workflows/` fenced untouched and proven so at two separated observations. The one workflow it wanted — a weekly advisory linkcheck job — was scoped in and **deferred at roadmap review** on a platform fact rather than discovered at execution: GitHub runs `schedule` only on the default branch and `workflow_dispatch` only for a file already there, so it was unprovable from an unmerged milestone branch. Both work requirements therefore sat in one phase, and the close prep phase carried only the merge (REL-14). Discovery was re-derived rather than inherited twice over: the DOC-24 surface list came from a fresh repo-wide grep and found a third surface, and the DOC-18 todo's recorded 4 `multiple toctrees` messages measured 5. The close handoff was written as an ordered step list with its conditional branches named, and the condition it predicted — `main` has moved — is the one that fired, because all five Dependabot PRs merged between planning and execution, overtaking the phase's own planning premise. `phase.complete` flipped the release checkbox again (9 of 10) and the fence caught it. Third consecutive `override_closeout` for the fingerprint reason, and the **first where every phase in the milestone went stale**, including the final one by its own tracking commits |
| v0.9.6 | ~7 days | 2 (no insertions) | **First milestone in four to publish**, and the one that carried three completed-but-unreleased milestones out with it under a single `## [0.9.6]` heading — so `0.9.3`, `0.9.4` and `0.9.5` join `0.9.1` as permanently unclaimed on PyPI. The defect was a node with no handler at all: docutils auto-detects any `>>>`-led block, so no author can opt out, and the acceptance fixture was therefore **this project's own documentation build** rather than a scratch project. The milestone's distinguishing move happened at the close, not during it: the pre-close artifact audit surfaced two todos that had been deferred under the prep-only fence, and re-reading the fence's *stated reason* showed it had lapsed — `origin/main` had moved and `main` is `strict`, so the CI re-dispatch the fence was protecting against was already mandatory. Both closed as quick tasks for no extra cost, and one of them mattered: the todo's own proposed `linkcheck_anchors_ignore_for_url` pattern would have been a **silent no-op**, because Sphinx strips the fragment before matching, and a bare `pypi\.org` entry would have prefix-matched every PyPI project page. Reading `sphinx/builders/linkcheck.py` at planning time caught both, and the fix was proved by a differential before/after status diff with the two tag-URL records *expected* to stay broken — producing the project's first unconditionally clean `tox -e linkcheck` (96/96, exit 0) once the tag existed. REL-16 ends a five-milestone run of declining a `### Known Limitations` section, with the candidate set narrowed from three defects to one on measurement because the other two were already fixed. `phase.complete` flipped the release checkbox again (10 of 11) and the line-scoped fence caught it while letting REL-16's legitimate close through. Fourth consecutive `override_closeout` for the identical fingerprint reason, and the second where every phase went stale. `create-release` ran on a v0.9.x tag for the first time and succeeded |
...
| v0.9.6 | 1569 passed / 1 skipped (up from 1543); + `tests/test_doctest_block_render_gate.py`, eleven tests over three masters driving a real `typst.compile()` in **both** contexts the defect actually breaks — a plain-paragraph doctest block and the definition-list / field-body shape — each recorded observably RED against the pre-handler translator, and `tests/test_docstring_rest_census_guard.py`, which parses every `typsphinx/*.py` docstring through napoleon + docutils so the reST-error census cannot silently regress | full suite green; **both documentation builds clean from a removed `docs/_build` with zero warnings each** (down from 3 and 5), the first time either has been at zero; `tox -e linkcheck` **96/96 `working`, exit 0** — the first unconditionally clean run in the project's history; CI 15/15 on release PR #156 including both `windows-latest` and both `macos-latest` lanes; `release.yml` run `35730551619` success on every job | 0 new runtime **and dev** deps; no new `typst_*` config value; `@preview` count still four with no version change, so the three-way sync guard was green throughout and untouched. The product delta is +64 lines across two files in `typsphinx/`, plus this project's first `linkcheck_*` key in `docs/source/conf.py` |
| v0.9.3 | 1547 passed / 1 skipped (Phase 69, twice, once under `LC_ALL=C`); `tests/test_toolchain_config_gate.py` re-pinned to `tox-uv` (the Phase 45.2 gate inverted) plus prose-only test edits proven by masked-AST hash (68-02); no product tests added, because no product code changed | every documented gate now runs through the shims on the maintainer's NixOS machine, including every tox environment and a real `docs-pdf` PDF; CI 12/12 on all three milestone dispatches and 15/15 checks on PR #143 | 0 new runtime deps; `tox-uv-bare` → `tox-uv` in the dev extra (a drop-in, released in lockstep); no new `typst_*` config value; `@preview` unchanged; `typsphinx/` untouched |
| v0.9.4 | 1547 passed / 1 skipped (1548 collected), identical before and after the conversion and again at Phase 71, twice, once under `LC_ALL=C`; no product tests added, because no behaviour changed — the evidence is the five QUA-12 legs, not new tests; 4 test modules converted with zero assertion edits | `ruff check .` now enforces `UP006`/`UP035` repo-wide; docs warnings unchanged at 3 / 5 from clean builds; CI 12/12 on both milestone dispatches and 15/15 checks on PR #145 | 0 new runtime or dev deps; no new `typst_*` config value; `@preview` unchanged and the version-sync import strings byte-identical; `typsphinx/` changed in annotations and typing imports only |
...
31. For a refactor that promises "nothing changes", the evidence is the deliverable. Split it into independent legs that each see a different failure, and prove every harness can fail before trusting it to pass. *(v0.9.4 — five QUA-12 legs, the masked-AST harness piloted on two files with two must-fail controls)*
32. A workflow file cannot be scheduled or dispatched from the branch that introduces it — GitHub reads both triggers from the default branch only. Decide at scoping whether to plan a side PR or defer; discovering it at execution costs the milestone the requirement. *(v0.9.5 — QUA-08 deferred at roadmap review on this fact, and the environment it would call shipped anyway)*
33. A phase planned days before it executes must re-measure its external premises. Phase 73 was planned around ordering five Dependabot PRs after the milestone PR; all five had already merged, and the live `gh` re-read, not the plan's prose, was authoritative. *(v0.9.5)*
