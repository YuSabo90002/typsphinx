...
## Current Milestone: v0.8.0 multi-master composition
...
**v0.8.0 in progress — Phase 47 complete 2026-08-12 (14 plans, 10/10 requirement IDs, verification
...
<!-- Scoped 2026-08-11 for v0.8.0. `.planning/REQUIREMENTS.md` is the authoritative, REQ-ID'd list;
     this section only ever carries the active milestone's headline commitments. -->

**v0.8.0 multi-master composition** — a `typst_documents` configuration with more than one master
produces a complete PDF for each:
...
- [ ] A cross-document reference degrades at compile time via a `context` + `query` label-existence
      ...
      compile once the include graph is fixed
- [ ] A converted image rehomed to `images/<basename>` no longer collides with a real source image at
      `<srcdir>/images/<basename>`, and an absolute URI outside `doctreedir` no longer escapes `outdir`
- [ ] v0.8.0 released to PyPI with a curated CHANGELOG entry calling out the output-shape change
...
- **Open defects the project knows about and chose to ship:** three `typst_documents`-modelling defects and two `TypstBuilder._track_image()` defects, all enumerated in `46-HANDOFF.md` § "Deferred by decision, not oversight" and carried into this file's Active candidates. The major `_track_image` one is a regression in failure mode — silent wrong picture where the same project used to abort loudly.
...
---
*Last updated: 2026-08-12 — **Phase 47 complete** (Two-Layer Output — Content/Wrapper Split, Target-as-Path, Collision Detection; 14 plans, 10/10 requirement IDs, `47-VERIFICATION.md` `passed` 12/12). The unit of output is now two layers: a docname-named, template-less **content** `.typ` per document, and a **wrapper** `.typ` per `typst_documents` entry carrying the template and the include of its master's content — so `writer.py`'s `_is_master_document()` output-shape binary is gone (confirmed by repo-wide grep, per milestone invariant #4, not by reading the file). **B-1** (`file not found` from the include-path vs. output-filename mismatch) and **B-2** (an included master re-expanding its title page and `#outline()` mid-body) are both closed on recorded-RED fixtures. **OUT-01 is a deliberate reversal of v0.7.1 Phase 44's D-05/D-06/D-07**, stated as such in the phase artifacts so the executor would not treat the existing guard code as sacred: a target is a path relative to the output directory, so a bare name writes at the output root and an explicit path writes where the user asked — while OUT-02 keeps the security half (`..`, absolute and drive-qualified targets still refused with a warning and a safe fallback). The collision work rode with the split rather than following it, because the split *creates* the hazard; BLD-02/03/04 now route through ONE pre-write validator over a single `_is_usable_typst_documents_entry()` predicate. **That predicate is the phase's durable lesson.** It was introduced wired into the four sites resolving a wrapper *path*, and verification found a fifth — the include-set computation deciding whether a cross-reference emits a real `link(<label>)` or degrades to plain text — still answering the same question with a bare `if entry` truthiness test: a BLOCKER (gap 9b / CR-01) reproduced end-to-end as both a silent dangling label and an uncaught `TypeError`. Promoting a check to a named predicate does not eliminate the drift it exists to prevent unless every consumer is enumerated and the docstring's count is kept honest — it said FOUR when there were five. Two gap-closure plans (47-13, 47-14) closed it and deleted a second dead resolver rather than commenting it, with the retargeted assertions' expected values verified verbatim against the pre-fix tree so the retarget could not launder a behaviour change into a green suite. Composition *semantics* were deliberately left untouched for Phase 49. Milestone invariant #5 paid again: the branch reached `origin` with a completed cross-OS CI run in this phase, not at the release PR. Next: **Phase 48 — Compile-Time Cross-Reference Guard**, which must land no later than Phase 49. Prior footer retained below.*
