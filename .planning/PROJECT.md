...
`v0.9.1` tag, locally or on the remote, no GitHub Release, and no release PR. `pyproject.toml:7` is
...
- **Completed-but-unshipped state (2026-08-30, v0.9.1):** PyPI still carries `typsphinx 0.9.0`. There is no `v0.9.1` tag, no GitHub Release, and no release PR — v0.9.1's product work sits on `gsd/v0.9.1-windows-path-correctness`, unmerged, with `pyproject.toml` still at `0.9.0` and the milestone's CHANGELOG bullets under `## [Unreleased]`. Four bundled `@preview` packages unchanged. **Zero runtime dependencies added since v0.6.0 and no new `typst_*` config value** — this was a bug-fix round by construction, and the one new module (`typsphinx/pathfmt.py`) is a zero-import stdlib leaf. Read the Docs `en`/`ja` still serve 0.9.0.
...
|----------|-----------|---------|
| **Cancel the v0.9.1 release outright and skip the version number — the next published release is 0.9.2** (owner, 61-CONTEXT D-02, 2026-08-29) | An owner report on 2026-08-29 surfaced a `blocker`: an image not first in its paragraph makes Typst refuse the file, so `-b typstpdf` writes no PDF for any master. Publishing a release that cannot compile a document containing an inline substitution image was declined. The number is skipped rather than held, so no `v0.9.1` tag will ever exist | ✓ Good — the first close in this project's history to publish nothing, and the constraint was honoured completely: no tag, no PyPI upload, no GitHub Release, no PR, no translations-repo pin advance. Verified live at four separated observations |
