...
---
*Last updated: 2026-07-11 after Phase 8.1 (admonition-rendering fix — translator markup/code-mode mismatch) complete — the tracked Phase-7 follow-up bug is fixed: admonition bodies now render typeset prose (code-mode `clue_type({...})` form) with inline-markup titles preserved, 5 new admonition types added (D-06), unit asserts strengthened to structural checks (D-03/D-05), and a real D-04 acceptance gate (`tests/test_pdf_render_gate.py` + `tox -e docs-pdf` compile → `pypdf` text-extraction → no literal-source leak) proves it in a real render. Full suite 411/411 green; black/ruff/mypy clean on `release/v0.5.0`; gentle-clues 1.3.1 / `@preview` versions unchanged. (Phase 8 before it: Sphinx 9 / docutils 0.22 API compatibility, suite green under a permanent `filterwarnings` deprecation guard.) Next: Phase 9 (CI matrix green + smoke test + guardrails), then Phase 10 (v0.5.0 PyPI release)*
