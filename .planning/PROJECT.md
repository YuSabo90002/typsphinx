...
**v0.9.0 (shipped 2026-08-22)** made the template a per-document choice. A new `typst_document_templates` registry names template definitions — each carrying `template` (a local `.typ` path) **xor** `package` (a Typst Universe spec), plus an optional `template_function` — and a `typst_documents` entry's fifth element, until now documented as "accepted and ignored", names the registry key that entry uses. Several entries can share one key, and a four-element entry behaves identically to one whose fifth element is `"typst"` — the built-in key, resolved by the same rule as any declared key and falling back to the same global configuration it always did, so the registry is purely additive. The output shape changed to make it work: every used key's template bundle directory is copied **wholesale** to `<outdir>/_template/<key>/`, the built-in key under the same rule with no exception, so template-relative asset references resolve for the first time; the single shared `<outdir>/_template.typ` is gone, along with `typst_template_assets` and four now-dead code paths. Misconfiguration fails **before any `.typ` file is written**: an unregistered key, `template` and `package` in one entry, a user-defined `"typst"` key, a template path that escapes its bundle, and a bundle directory colliding with Sphinx's own `templates_path` each abort the build by name. This is a **breaking** minor release on two independent axes — where the template file lands, and the removal of `typst_template_assets`.

**v0.9.1 (completed 2026-08-30 — deliberately never published)** was a Windows path-correctness bug-fix round, and the first milestone in this project's history that ships nothing. Three latent defect families closed on the product side: `_escapes_outdir()` now applies its `isabs`/drive-qualified terms to the backslash-normalized string rather than the raw stem, matching the idiom its sibling predicate already shipped; `_track_image()`'s escape branch became safe end to end — the relocation key normalizes the basename while still hashing the raw URI, the `{digest}-{basename}` component is bounded to 255 UTF-8 bytes on character boundaries with the digest anchor kept whole, and `visit_image()` routes the emitted URI through `escape_typst_string()` at both emission sites, so a Windows-shaped absolute image URI now survives into an `image("...")` a real `typst.compile()` accepts; and every path-valued diagnostic interpolation in `builder.py`, `writer.py` and `template_registry.py` routes through one new zero-import leaf module, `typsphinx/pathfmt.py::quote_path()`, which reproduces `repr()`'s delimiter-selection rule without its backslash doubling. Every fix was gated RED-first against the unfixed tree, and POSIX output was proven byte-identical the way v0.9.0 proved it — by zero pre-existing test edits, measured rather than asserted. **The release itself was then cancelled.** An owner report on 2026-08-29 surfaced a blocker unrelated to this milestone's own work: an image that is not the first thing in its paragraph is emitted adjacent to the preceding code-mode expression, so Typst aborts with `expected semicolon or line break` and `-b typstpdf` raises `ExtensionError` and writes no PDF for any master — measured byte-identical to the `v0.9.0` tag, so pre-existing, not a regression. Publishing a release that cannot compile a document containing an inline substitution image was declined. `pyproject.toml` stays at `0.9.0`, the milestone's CHANGELOG bullets wait under `## [Unreleased]`, no tag exists, and **the next published release is 0.9.2**.
...
milestones' worth of change already sitting in `## [Unreleased]` — as v0.9.6.

**Target features:**

- **TRN-01 — a `doctest_block` handler.** `doctest_block` is not a directive: docutils' parser
  auto-detects any `>>>`-led block (`docutils/parsers/rst/states.py:1251` `'doctest': r'>>>( +|$)'`,
  built at `:1698`), so a Google/NumPy-style `Examples:` docstring produces one with no markup
  opt-out available to the author. `TypstTranslator` has no handler (`grep -n doctest
  typsphinx/translator.py` is empty), so the node falls through `unknown_visit()`
  (`translator.py:5819`); its `Text` children are still visited and re-emitted as ordinary inline
  text, so every line break is lost. The owner's choice (2026-09-16) is to render it exactly as a
  code block does — which is what Sphinx's own writers do: `visit_doctest_block =
  visit_literal_block` in `sphinx/writers/latex.py:2324` and `sphinx/writers/texinfo.py:812`, and a
  delegating call in `sphinx/writers/html5.py:665`. One wrinkle is already measured:
  `HighlightLanguageTransform` assigns `node['language']` only to `literal_block`, never to
  `doctest_block`, so a bare alias would reach `translator.py:2570`'s `node.get("language", "")`
  with an empty string and emit an unhighlighted fence — the handler must supply the language
  itself. Both ` ```pycon ` and ` ```python ` compile under a real `typst.compile()`, so the tag is
  a highlighting-quality choice, not a validity one.
- **The docstring reST errors.** The same clean `-b typst` build reports docutils `Unexpected
  indentation` (`TypstTranslator.visit_toctree` docstring, lines :5 and :21) and `Block quote ends
  without a blank line` (:6). `milestones/v0.9.5-REQUIREMENTS.md`'s Future section names exactly
  these as what a docs warnings gate would first need fixed.
- **Publish v0.9.6.** `pyproject.toml` bumped from `0.9.2`; the six `## [Unreleased]` bullets — one
  Added, four Changed, one Fixed, five of them explicitly "This has no effect on installing or
  using typsphinx" — promoted into `## [0.9.6]` alongside this milestone's own; then tag → PyPI →
  GitHub Release. The version was chosen by the owner on 2026-09-16 to keep the milestone number
  and the released version aligned, the rule every milestone here has followed; `0.9.3`, `0.9.4`
  and `0.9.5` stay permanently unclaimed on PyPI exactly as `0.9.1` already is (the tag series
  jumps `v0.9.0` → `v0.9.2`).

**The acceptance gate is this project's own documentation.** No scratchpad project and no
`sphinx-autoapi` install is needed to reproduce the defect: `docs/source/api/index.rst`'s
`automodule` directives already pull `typsphinx.writer`'s two public `Examples:` docstrings, so it
is live in `tox -e docs-pdf` output today. Measured on `main` at the scoping session (2026-09-16), a
clean `sphinx-build -b typst docs/source <tmp>` reports `build succeeded, 5 warnings.`, of which two
are `unknown node type: <doctest_block ...>` in `api/index`, and `api/index.typ:811` carries a
six-line, three-prompt example as a single `text("...")` run. After the fix those two warnings must
be gone, `api/index.typ` must preserve the example's line structure, and the result must survive a
real `typst.compile()` — the standing GATE-01 bar.

**Exposure is larger than the docs show.** Only two blocks reach the built documentation because
`automodule`'s `:members: :undoc-members:` excludes private functions. The source tree holds 51 `>>>`
prompts — 32 in `typsphinx/builder.py` (mostly `_`-prefixed helpers), 11 in `translator.py`, 8 in
`writer.py` — and the original 2026-09-13 measurement that filed TRN-01 counted 17 `doctest_block`
warnings in a `sphinx-autoapi` build of this same package.

**Open at scoping.** Publishing forces the `### Known Limitations` CHANGELOG question that v0.9.5
could leave alone because it published nothing; the candidate set is the carried major defects
(NUM-01, the converted-image rehome collision, the `typst_documents` duplicate-target cluster).
`release.yml`'s `create-release` job has real end-to-end evidence from the v0.7.1 publish (run
`31462027486`) and v0.8.0 (run `31861043480`), but has never run on a v0.9.x tag.

## Shipped Milestone: v0.9.5 Docs Link Check and Navigation (completed 2026-09-16 — merged, NOT published)

**Goal achieved; nothing published.** Two phases (72–73), 13 plans, 31 tasks, **4/4 v1
requirements complete**. REL-14, the milestone's one irreversible step, was the merge itself: PR #151
to `main` as `43fd7c13`, 15/15 checks green including both `windows-latest` and both `macos-latest`
lanes, with no tag, no PyPI upload, no GitHub Release and `pyproject.toml` still at `0.9.2`.
`override_closeout`: both phases' verifications read fingerprint-stale after later `.planning/`
tracking commits touched files they cover (Phase 72 by its own `phase.complete`/UAT/security
commits, Phase 73 by its code-review, `phase.complete` and third-fence-observation commits); both
VERIFICATION.md files themselves read `passed` (5/5 and 4/4, zero overrides), and the same-day
`v0.9.5-MILESTONE-AUDIT.md` (`tech_debt`, no requirement, integration or flow gaps) stood in.

**What it delivered.** (1) **`tox -e linkcheck`** (QUA-13, DOC-24) — a new `[testenv:linkcheck]`
shaped exactly like `docs-html` and kept outside `env_list`, green from day one at 95/95 links
`working`, listed on all three surfaces a fresh repo-wide grep found. (2) **The sidebar matches the
hierarchy** (DOC-18) — the five duplicated section children are gone from the root toctrees,
`multiple toctrees` messages 5 → 0, docs-html warnings 3 → 3 and docs-pdf 5 → 5, each page still
included exactly once under `-b typst`. (3) **The close proven unpublished-shaped twice**, at
observations 33 minutes apart, each with positive controls on version, tag, PyPI, GitHub Release and
the release workflow, and a milestone fence showing `typsphinx/` and `.github/workflows/` untouched
for the whole milestone.

**Accepted and carried.** QUA-08, the weekly advisory CI job that would call `tox -e linkcheck`, was
scoped in and then deferred to Future by the owner at roadmap review (2026-09-13): GitHub runs
`schedule` only on the default branch and `workflow_dispatch` only for a workflow file already
there, so it could not be proven on the unmerged milestone branch. Phase 73 ran neither Nyquist
validation nor the security audit. `[testenv:linkcheck]` carries no failure-tolerance override, so a
transient external URL fails the env locally with no guidance. The release-checkbox auto-flip fired
again at Phase 73's close — 9 of the 10 prior release-prep closes — and was caught by the
constraint-10 fence and reverted before commit; the fence stays in place.
...
**Goal achieved, and published.** `typsphinx 0.9.2` is on PyPI. Two phases (62–63), 10 plans, 29
tasks, **7/7 v1 requirements complete** — the first `verified_closeout` since v0.6.4, and the first
milestone in this project's history to complete inside a single calendar day (~11 hours). It carries
v0.9.1's entire completed-but-unpublished output as well as its own fix, under one `## [0.9.2]`
CHANGELOG heading. **No `## [0.9.1]` heading and no `v0.9.1` tag exist**, because that release was
cancelled rather than skipped.

**What it fixed.** An image node that was not first in its container was emitted adjacent to the
...
content file, so one refused file poisons every master that transitively includes it. The defect was
measured **pre-existing against the `v0.9.0` tag** (D-06 at the v0.9.1 close), reachable through
stock reST, and live in the published 0.9.0 the whole time; it was found by the owner using the
tool, not by any gate. `visit_image()`/`depart_image()` now join the separator triad the rest of the
translator already runs on — `_add_paragraph_separator()`, `_emit_inline_concat_separator()`, and
the `in_list_item` / `list_item_needs_separator` pair — as a **9-line pure insertion with zero
deletions**, leaving both the `in_figure` and `else` branch bodies textually unmodified and
introducing no new line-boundary predicate.

**The mechanism was amended before a line was written.** A live 27-document / 18-master probe
measured that driving the triad from the non-`in_figure` branch *alone*, as the requirement first
specified, leaves **4 of 18 masters still refused**: both legend shapes (a legend image has
`self.in_figure == True` and never reaches that branch), the field-list-body concat shape (where the
unconditional trailing newlines break the concat with a *new* refusal, `cannot apply unary '+' to
content`), and `index` transitively. The leading half was hoisted above the `if self.in_figure:`
split and the trailing half made concat-aware — 18/18 compiling, with the literal
`in_figure`-branch-unmodified success criterion still holding. This is the second consecutive
milestone in which a locked decision was falsified by measurement, put to the owner, and closed with
an `AMENDED` block rather than read to fit.

**The gate is a real compile, and it was RED first.** One module drives `typst.compile()` through
`-b typstpdf` over 16 measured failing shapes and 9 that must keep passing, from a single build
invocation. Nine existing string-level image tests never saw this defect because the emitted string
*looks* plausible and only the parser rejects it. Before the fix landed, `typsphinx/translator.py`
was restored from the phase base SHA, the 17-master aggregate `ExtensionError` transcribed verbatim,
the fix restored, and `git status --porcelain` recorded empty. Eight of the nine PASS shapes are
byte-identical against committed goldens; the ninth gains exactly one empty line and is pinned by an
exact-delta assertion rather than softened to "compiles". **Zero pre-existing test edits**, measured
with `git diff --name-status` across the 20 files carrying the 144 `image(` matches.

**Published, and measured.** Branch merged to `main` as `45962faa` via PR #136 (15/15 checks green,
both `windows-latest` and both `macos-latest` lanes); tag `v0.9.2` on that merge commit;
`release.yml` run `33318905691` with `Validate Release`, `Build Distribution`, `Publish to PyPI` and
`Create GitHub Release` all `success`. The GitHub Release body is **byte-identical** to
`scripts/extract_changelog_section.py 0.9.2` — 4083 bytes / 54 lines, `diff` empty against the
published body truncated to that line count, zero `### Planned for Future Releases` leakage.
`typsphinx-doc-translations` advanced its submodule pin to exactly `45962faa` via its own dispatched
`update-pin.yml` (`fcf66da4`) and carries a matching `v0.9.2` tag.

**A post-plan gate caught a false claim inside the release notes themselves.** The curated
`## [0.9.2]` intro asserted "the runtime changes are confined to `typsphinx/translator.py`, with no
other file under `typsphinx/` touched" — falsified by that same evidence file's own
milestone-invariant sweep showing five files changed since 0.9.0. `63-REVIEW.md` CR-01 (Critical)
and `63-VERIFICATION.md`'s SC#2 block found it independently, *after* the original extractor
inspection had passed the structural checks clean. The blanket sentence was deleted, a narrower
measured version re-scoped into the one bullet it is true for, and the byte-identity proof re-taken
against the corrected text — which is the text that shipped.

**The release-checkbox fence held for the second time running.** REL-11's SHA-256 of
`.planning/REQUIREMENTS.md` was verified at phase head, at phase close, after the gap-closure
commits, inside `63-HANDOFF.md`, and once more by the operator at the close **after** all
...
closes; it did not flip here. REL-09/REL-10/REL-11 were checked by the operator at the close, on the
observed publish.

**REL-04 closed on evidence this release generated.** `Create GitHub Release` — the job that failed
with `uv: command not found` at the v0.7.0 tag push and was carried unproven through three
milestones — completed `success`, read from the job's own conclusion rather than inferred from the
run status. Third consecutive green after v0.8.0 and v0.9.0. Its todo is now in `todos/completed/`.

**Not scoped into v0.9.2**, carried forward unchanged: the `numref` per-master divergence, the
dependabot `uv.lock` `--locked` mismatch, `translator.py:5047,5152`'s hardcoded-delimiter debug logs
(MSG-06), WR-02 (`templates_path` resolved against `srcdir`), 54.1 WR-01 (the tripled "Custom
template not found" warning), the `sphinx-build -b linkcheck` CI job, the typing modernization
(`UP006`/`UP035`), `ruff`-on-NixOS, the root-toctree HTML sidebar duplication, and SEED-001,
SEED-003, SEED-004.


## Shipped Milestone: v0.9.1 Windows path correctness (completed 2026-08-30 — NOT published)

**Goal achieved on the product side; the release was deliberately cancelled.** 4 phases (58, 59,
60, 61), 17 plans, 48 tasks, **10/11 v1 requirements complete** — the eleventh, REL-09 (publish to
PyPI), is deliberately unmet and carries forward to v0.9.2 with its literal wording unchanged
(D-08). `/gsd-complete-milestone` performed **no tag, no PyPI publish, no GitHub Release, and no
pull request** for this milestone (D-02, D-12) — the first close in this project's history to
publish nothing.

**Next milestone: scoped 2026-08-30 as v0.9.2** — see `## Current Milestone` above. Its first
requirement is the inline-image blocker
(`.planning/todos/pending/2026-08-29-inline-image-in-paragraph-emits-unseparated-expression.md`),
the reason this release was withheld, and it carries REL-09 forward as the 0.9.2 publish.

The original scoping record follows, retained unchanged.
...
master. 6 phases (53, 54, 54.1 inserted, 55, 56, 57), 42 plans, 154 tasks, 26/26 v1 requirements
complete.

**Next milestone: not yet scoped.** Run `/gsd-new-milestone` — it re-scopes this section, the
`### Active` requirements list below, and `.planning/REQUIREMENTS.md` (removed at this close).
...
`0.9.2`, `README.md:347` reads `Stable (v0.9.2)`, and the GitHub Release `v0.9.2` carries a body
...
`### Planned for Future Releases` leakage. `typsphinx-doc-translations` is pinned to exactly
`45962faa` (`fcf66da4`) and tagged `v0.9.2`. **Read the Docs `stable` was not re-measured at this
close** — both projects' Default Versions have been `stable` since v0.6.4 and have needed no re-flip
since, but that is an expectation here, not an observation; the procedure is `63-HANDOFF.md` § 5.

**Two phases, 10 plans, 29 tasks, 7/7 v1 requirements complete.** Closed as `verified_closeout` —
the first since v0.6.4, after seven consecutive `override_closeout`s. Both phases carry
`phase_complete: true` and `verification_status: passed`; UAT closed 25/25 with zero issues; the
pre-close artifact audit reported **no open items** (15 previously acknowledged items from earlier
closes remain suppressed). **No `v0.9.2-MILESTONE-AUDIT.md` was produced** — `/gsd-audit-milestone`
was not run, so cross-phase integration and E2E coverage rest on the two phase verifications, the
UAT, and the real-compile gate rather than on a separate audit pass. Recorded as an absence, not
claimed as a pass.

**What is now true that was not.** A document that places an image anywhere other than first in its
container compiles. Before this release it did not — `sphinx-build -b typstpdf` raised
`ExtensionError` and wrote no PDF for **any** master in the project, including masters containing no
image at all, and that was true of the *published* 0.9.0 for its entire life. The fix is a 9-line
pure insertion in `visit_image()`/`depart_image()` routing through the translator's existing
separator triad, gated by a real `typst.compile()` over 16 measured failing shapes and 9 that must
keep passing, recorded RED against the unfixed tree first. `0.9.2`'s release notes name the upgrade
explicitly: *"A project built with the published 0.9.0 release produced no PDF for any master
document when an image was not first in its container, and 0.9.0 users should upgrade to this
release."* That sentence is the first public-surface disclosure this defect has ever had — D-05's
silence at the v0.9.1 close is discharged by fixing and shipping, not by an erratum.

**The milestone's carrying lesson is that the last gate before the publish is the one that reads the
publish's own output.** `63-REVIEW.md` and `63-VERIFICATION.md` independently caught a false blanket
claim — "the runtime changes are confined to `typsphinx/translator.py`, with no other file under
`typsphinx/` touched" — inside the curated `## [0.9.2]` section, *after* the extractor had been run
and its structural checks had passed clean. The same evidence file's own invariant sweep contained
the five-file diff that falsified it. Structural correctness of a release note is not the same
property as its claims being true, and only a gate that reads the prose finds the difference.

**Zero runtime and dev dependencies added, no new `typst_*` config value, no `@preview` bump.** The
four bundled packages and their three-way version-sync surface are untouched. Code delta outside
`.planning/`: 46 files, +1,051 / −11 — of which `typsphinx/` is **+23 in one file**.

**Prior state, retained for reference — v0.9.1 COMPLETE 2026-08-30 and NOT SHIPPED.** PyPI still carried `typsphinx 0.9.0`; there was no
`v0.9.1` tag, locally or on the remote, and no GitHub Release. The work *is* on `main` — merged
after the close as `9db2274c` via merge-only PR #135, 15/15 checks green — but nothing about that
is a release. `pyproject.toml:7` is
still `0.9.0` and `README.md:347` still reads `Stable (v0.9.0)`. This is the **first milestone in
this project's history to close without publishing**, and the inversion is deliberate: `61-HANDOFF.md`
opens with the negative rather than with a publish checklist, breaking a run of seven consecutive
handoffs that opened as one.

**Why.** On 2026-08-29, mid-milestone, the owner reported that `par(image(...))` patterns exist and
break the build. Reproduced and root-caused the same day: an image node that is not the first thing
in its paragraph is emitted directly adjacent to the preceding code-mode expression on one line, so
Typst refuses the file with `expected semicolon or line break` and `-b typstpdf` does not degrade —
it raises `ExtensionError` and writes **no PDF for any master document in the project**. Severity
`blocker`. It is **pre-existing, not a regression**: `git diff v0.9.0..HEAD -- typsphinx/translator.py`
is 25 lines, all IMG-05's `escape_typst_string()` call, and `visit_image()`'s missing leading
separator is byte-identical to the `v0.9.0` tag (D-06). The blocker was deliberately **not fixed in
this milestone** (D-07) — Phase 61 was prep-only and `translator.py` was out of both the fence and
the requirement set — and deliberately **not disclosed on any public surface** (D-05). It carries to
v0.9.2 as that milestone's first requirement.

**Four phases, 17 plans, 48 tasks, 10/11 v1 requirements complete.** All four phases
`phase_complete=true` / `verification_status=passed` — the first close in six where that was true
*and* a `MILESTONE-AUDIT.md` was actually produced. `v0.9.1-MILESTONE-AUDIT.md` reports
`status: tech_debt` with `fail_gate.triggered: false`: requirements 10/11, phases 4/4, integration
11/11, flows 1/1, and REL-09 classified `deferred` rather than `unsatisfied` because the owner
amended the definition of done rather than the requirement text. Closed as `override_closeout` on
that one deliberate gap plus 16 acknowledged open artifacts.

**Phase 61 held a fence that has slipped at five consecutive prior release-prep closes.** REL-09's
checkbox was guarded by a SHA-256 of `.planning/REQUIREMENTS.md` recorded at phase head
(`4682f8cd…`), re-verified at phase close, again inside `61-HANDOFF.md`, and once more by the
operator at this close — MATCH every time, with `git tag -l 'v0.9.1'` empty at each observation.
SC#4's no-irreversible-action proof rests on two observations separated by 38m16s spanning two waves
and a full 3-OS CI dispatch, each network probe carrying a real positive control, and the empty
`typsphinx/` diff proven non-vacuous by a live widened control. CI acceptance run `33260111745`:
12/12 `success`, both `windows-latest` lanes included. Local: 1513 passed / 5 skipped, `black`,
`mypy` and the version-sync guard family clean.

**The milestone's carrying lesson is that a locked decision can be falsified by measurement.**
`59-CONTEXT.md` D-01 and ROADMAP SC#2 both predicted the unfixed tree would be refused with `path
must not contain a backslash`; it is actually refused with `unclosed delimiter`, because the unfixed
pipeline emits a literal carrying a raw backslash **and** a raw unescaped `"` at once and the quote
ends the string before the semantic check runs. D-01's four probe runs had each carried only one
defect. Re-measured independently, put to the owner, and closed with an `AMENDED` block rather than
silently satisfied — the second consecutive milestone in which a success criterion was amended on
evidence instead of being read to fit. Two further defects were found by gates *after* their phases'
plans all reported complete: Phase 59's own code review found a blocker in
`_bound_relocation_component()` (one reserved *byte* empties a multi-byte leading character), and
Phase 60's repo-wide SC#2 grep found a **fourth** module carrying the MSG-02 defect shape
(`translator.py`'s relative-path DEBUG logs) — filed as a todo rather than fixed out of scope.

**Prior state, retained for reference — v0.9.0 SHIPPED 2026-08-22.** PyPI `typsphinx 0.9.0` is live (wheel 187,400 B + sdist 809,731 B),
published by release run `32560457509` after owner approval of the `pypi` environment. PR #134
merged to `main` as `68b92e24` with **all 15 checks green** — a fresh full matrix on the exact PR
head, both `windows-latest` lanes included — and the tag was pushed on that merge commit. The
release ran `validate` → `build` → `publish-pypi` → `create-release` all `success`, **including
`create-release`**, the job that failed at the v0.7.0 close and was therefore observed directly
rather than assumed. The GitHub Release body's first 123 lines are **byte-identical** to
`scripts/extract_changelog_section.py 0.9.0`'s stdout (`diff` clean), with zero commit-dump-shaped
lines in that window; it carries all three assets (`.whl`, `.tar.gz`, and the tag-time
`typsphinx.pdf`, 2,754,479 B, attached by `docs.yml`). `typsphinx-doc-translations` advanced its
submodule pin to the v0.9.0 merge commit `68b92e24` through its own dispatched `update-pin.yml`
(run `32566262655`) — not by a hand-made clone-edit-push, for the second consecutive close — and
carries the annotated tag `v0.9.0` on the resulting commit `f9390d4b`. Read the Docs `en` `stable`
resolves from the root, reports `0.9.0`, and serves its PDF (2,760,619 B, `application/pdf`); `ja`
`stable` advanced to `ref: v0.9.0` once the translations repository's tag landed, reports `0.9.0`,
and serves its PDF (2,963,030 B). No Default Version flip was needed on either project — the sixth
consecutive close at which none was required.

**Six phases, 42 plans, 154 tasks, 26/26 v1 requirements complete, zero known gaps.** Closed as
`override_closeout` without a `MILESTONE-AUDIT.md` — the **sixth consecutive** close taken that way,
a pattern that has outlived being a default. 13 open artifacts were acknowledged and deferred, every
one of the 10 todos already dispositioned with a reason in `57-HANDOFF.md`.

**The milestone's defining episode was a defect diagnosed wrong twice.** Two full CI matrices
(`31956166848`, `31959060298`) were spent on a single `windows-latest` assertion read as a path
**separator** problem. It was an **escaping** problem: three pre-write template-path refusal messages
in `typsphinx/builder.py` interpolated PATH values with `!r`, and `repr()` doubles every backslash,
so no `str(Path(...))` assertion could ever match. Plan `57-05` **halted** rather than claiming its
authority gate; `57-11` fixed the product side, knowingly breaking the prep-only fence once by
explicit owner decision recorded as an `AMENDED` block naming SC#4, `57-08` and the phase verifier
as its readers; and fresh authority run `32557477023` returned 12/12 success on the post-fix tip.
**Three lessons carry forward.** *A local blind spot is worth closing before the next dispatch*:
`57-11` added a guard that drives the real message builders with a Windows-shaped path on this POSIX
host, proven load-bearing by a live revert-and-restore RED/GREEN. *A halt is retired on new evidence,
not retracted as a mistake*: `57-05`'s frontmatter moved `halted` → `complete` under a dated ADDENDUM
keeping the contemporaneous failure record verbatim. *A memory can falsify itself*: this milestone
measured on 2026-08-16 that `ruff` runs on this host and wrote it into a CONTEXT amendment; `57-09`
proved it false one week later — the main checkout's `.venv` holds an old runnable binary while every
freshly-provisioned worktree venv pulls a newer generic-linux wheel NixOS cannot exec, so measuring
`ruff` on the main tree can never detect the hazard. The owner's earlier refusal to close that todo
is what preserved the record.

**Ships with three known limitations**, all deferred by owner decision and recorded only in
`57-HANDOFF.md` § "Deferrals carried forward" and `.planning/todos/pending/` — the third consecutive
release to decline a `### Known Limitations` CHANGELOG section: the `templates_path` collision check
resolves against `srcdir` rather than `confdir` so `-c`/confdir projects stay uncovered (WR-02, and
the shipped CHANGELOG sentence reads as unconditional, which is stronger than a missing footnote);
the "Custom template not found" warning fires three times instead of two for one narrow shape; and
the pre-write refusal messages quote paths with a fixed `'...'` delimiter, so a path containing a
literal single quote closes the quote early.

Prior pre-publish state retained below.

**v0.9.0 was fully PREPPED and NOT PUBLISHED — Phase 57 (v0.9.0 Release Prep, prep-only) complete
and verified 2026-08-22.** 11 plans across 4 waves (two, `57-10` and `57-11`, added mid-execution
by owner decision), verification `passed` 5/5 must-haves with 0 gaps and 0 human-verification
items. The tree reads `0.9.0` in `pyproject.toml` / `uv.lock` / `README.md` with
`typsphinx.__version__` regenerated to match; `CHANGELOG.md` carries a curated `## [0.9.0]`
section with exactly four `**Breaking` bullets and a rolled-over tail link block; and
`57-HANDOFF.md` is the standalone six-item checklist `/gsd-complete-milestone` executes.
**REL-08 remains `[ ]`/Pending by design** — it closes on the publish, never in the phase.

**The phase's defining episode was a defect diagnosed wrong twice.** Two full CI matrices
(`31956166848`, `31959060298`) were spent on a single `windows-latest` assertion read as a path
**separator** problem. It was an **escaping** problem: three pre-write template-path refusal
messages in `typsphinx/builder.py` interpolated PATH values with `!r`, and `repr()` doubles every
backslash, so no `str(Path(...))` assertion could ever match. Plan `57-05` halted rather than
claiming its authority gate; `57-11` fixed the product side — knowingly breaking the prep-only
fence once, by explicit owner decision recorded as an `AMENDED` block naming SC#4, `57-08` and the
phase verifier as its readers — and fresh authority run `32557477023` returned **12/12 success,
both `windows-latest` lanes green**, on the post-fix tip.

**Three lessons carry forward.** **A local blind spot is worth closing before the next dispatch,
not after**: `57-11` added a guard that drives the real message builders with a Windows-shaped
path on this POSIX host, and it was proven load-bearing by a live revert-and-restore RED/GREEN,
so a third burned matrix is now impossible for this defect class. **A halt is retired on new
evidence, not retracted as a mistake**: `57-05`'s frontmatter moved `halted` → `complete` under a
dated ADDENDUM with the contemporaneous failure record kept verbatim, on the route its own SUMMARY
had prescribed. **A memory can falsify itself**: this milestone measured on 2026-08-16 that `ruff`
runs on this host and wrote that into a CONTEXT amendment; `57-09` proved it false one week later
— the main checkout's `.venv` holds an old runnable binary while every freshly-provisioned venv
pulls a newer generic-linux wheel NixOS cannot exec, so measuring `ruff` on the main tree can never
detect the hazard. The owner's earlier refusal to close that todo on the 2026-08-16 evidence is
what preserved the record. `phase.complete`'s REL-08 auto-flip fired for the **fifth consecutive**
release-prep close and was caught and reverted by `57-CLOSEOUT-GUARD.md`'s checksum, exactly as
SC#4 intends. Code review returned 0 critical / 1 warning / 1 info, the warning filed forward
rather than fixed, so the measured-green tree was not disturbed. Prior state retained below.

**v0.9.0 in progress — Phase 55 (v0.8.0-Derived Defects) complete 2026-08-16.** 4 plans across 3
waves, verification `passed` 5/5 must-haves, `55-REVIEW.md` 1 critical (re-rated to warning on
re-measurement) / 1 warning / 2 info. All five v0.8.0-derived requirements close on the **product**
side, each with a RED-recorded reproduction whose ancestry was checked against git history rather
than taken from prose. **XREF-05**: `_sanitize_label` encoded every Typst-invalid character as
`_u{cp:x}_` using only characters inside its own "safe" set, so an input already spelling that token
passed through untouched and `a/b` and `a_u2f_b` collapsed onto one label — a reference whose real
target was absent from the compiling master linked to the *other* document instead of degrading to
plain text. A `_LABEL_TOKEN_INTRODUCER_RE` pre-pass restores injectivity, proven by an exhaustive
decoder round-trip rather than by fixture pass. **BLD-07/BLD-08** (`translator.py`'s include-graph
derivation): edge keys now escape their own `#`/`>` separators, and the chain walk is bounded at
`_MAX_INCLUDE_CHAIN_DEPTH = 500` raising a named `ExtensionError` instead of a raw `RecursionError`.
**BLD-09/IMG-03** (`builder.py::_track_image`): the absolute-URI gate moves onto
`_is_absolute_image_uri()` and the escape branch's relocation key carries a SHA-1[:8] prefix over
the whole URI. Suite 1349 → **1366 passed, 5 skipped, 0 failed**; `black`/`ruff`/`mypy` clean; a
529-test regression gate over Phases 53/54/54.1 green. **Three things are worth carrying forward.**
First, **a success criterion was found to contradict itself and was amended on measurement, not
silently satisfied**: ROADMAP SC#4 named a predicate literally and, in the same sentence, required a
behaviour that predicate measurably does not produce (it evaluates `False` for exactly the
driveless-absolute and UNC shapes BLD-09 requires to reach the rehome branch). Plan `55-03` stopped
at a blocking decision checkpoint rather than resolving it; the owner chose the backslash-normalized
spelling and approved amending SC#4's wording, which `55-04` applied — so the shipped predicate and
the roadmap text now agree, and the amendment is traceable to the table it rests on. Second,
**the code review's one blocker survived its mechanism check but not its framing check**: CR-01's
Typst compile failure (`path must not contain a backslash`, from a non-normalized `path.basename()`
in the escape branch) reproduces exactly as described, but running the pre-fix `builder.py` from
`40b92fc6` side by side showed the identical failure existed before this phase — it is pre-existing,
not a Phase 55 regression, and falsifies neither SC#4 nor SC#5. It ships tracked, as a pending todo
carrying CR-01, WR-01's missing filename-length bound, and the absence of any escaping at
`visit_image()`'s `image("...")` emission site, cross-linked to the sibling `_escapes_outdir()` gap
`55-03` filed. Third, **the property that would have caught CR-01 is invisible to this phase's own
tests**: neither BLD-09 test renders or compiles its result, both stopping at `node["uri"]` — so the
filed todo requires whatever fix lands to carry a real `typst.compile()` gate.
...
- ✓ A `conf.py` can declare named template definitions and every malformed registry stops the build by name — v0.9.0 Phase 53 (TPL-01, TPL-03, TPL-04, TPL-05, CONF-14..18): `typsphinx/template_registry.py` resolves `typst_document_templates` once per build, each key carrying `template` (a local `.typ` path) **xor** `package` (a Typst Universe spec) plus an optional `template_function`. Validation is a fixed-order seven-case key-shape denylist plus the reserved-key check, the template/package xor, the CONF-17 path-arithmetic bundle-escape guard and a per-key existence check, all **accumulated into a single independent `ExtensionError`** rather than aborting on the first fault; the raw-exception surface (cross-drive absolute path, non-`str` key, truthy non-`dict` definition, a list where a dict belongs, a list/bytes `template` field) was closed in follow-up plans so no input shape produces a bare `AttributeError`/`TypeError`. The built-in `"typst"` key is resolved by the same rule as any declared key and falls back to the global configuration, and an unregistered key named by a `typst_documents` entry fails **before any `.typ` file exists**, in both master sort orders. Additivity was proven, not asserted: plan 53-01 captured SHA-256 hashes and PDF page counts across all four existing configuration shapes from real `-b typstpdf` runs *before any code changed*, and the tracer plan measured byte-identical output against that stored baseline — Validated in Phase 53 (53-VERIFICATION.md; 10 plans)

- ✓ Element [4] actually selects the template and the one bundle rule replaced four mechanisms — v0.9.0 Phase 54 (TPL-02, CONF-19, OUT-04..07, BLD-05, BLD-06): every used registry key's resolved template bundle directory is copied **wholesale** to `<outdir>/_template/<key>/` at `finish()` time, the synthesized built-in `"typst"` key under the same rule with no exception (closing a previously-unnamed whole-source-tree-copy hole), and every wrapper imports its own key by a root-absolute `/_template/<key>/<file>.typ` path that does not depend on the wrapper's nesting depth — so template-relative asset references resolve for the first time. Five `builder.py` methods were **deleted** rather than extended (the shared `_template.typ` writer plus the parallel asset-copy path with its three early returns and two explicit-list helpers), a fully-obsoleted 8-test module was removed after auditing every test against its coverage successor, and `typst_template_assets` was deregistered — connected in the same commit to `typsphinx/removed_config.py`, this codebase's **first Sphinx event-handler connection**, so a `conf.py` still setting any of three removed values gets a `config-inited` warning naming the value, its replacement, and the observable consequence instead of silently different output. The output-path collision guard widened from an exact-name claim on the deleted `_template.typ` to a first-segment prefix reservation over the whole `_template/` directory — Validated in Phase 54 (54-VERIFICATION.md; 7 plans)

- ✓ The two safety defects the wholesale-copy rule created were closed in an inserted phase, not deferred — v0.9.0 Phase 54.1 (WR-01, CR-01): a used key whose resolved bundle directory collides with Sphinx's own `templates_path` now refuses the build with an `ExtensionError` **before any `.typ` file is written** — closing a hole that would have republished a project's Jinja template directory into public build output while the published documentation actively recommended that layout — and a CONF-17 violation on the built-in `"typst"` key is caught pre-write instead of at `finish()`, after every content and wrapper file already existed. A build carrying two different pre-write failure kinds raises once with a byte-identical, declaration-order-independent aggregate message. Five new fixtures and thirteen new test methods pin all three path relations and all three non-refusing shapes. **Ships with WR-02 open and silent by decision D-09**: `_validate_used_template_paths()` resolves `templates_path` against `srcdir` rather than `confdir`, so a `-c`/confdir project is not covered — no CHANGELOG carve-out, no `### Known Limitations` section, no issue — Validated in Phase 54.1 (54.1-VERIFICATION.md `passed` 5/5; 5 plans)

- ✓ The five v0.8.0-derived defects closed on the product side, each with a RED-recorded reproduction — v0.9.0 Phase 55 (XREF-05, BLD-07..09, IMG-03): `_sanitize_label` made injective by re-escaping its own encoding token's introducing underscore, closing a label-collision **false negative** where `a/b` and `a_u2f_b` collapsed onto one label so a reference whose real target was absent linked to the decoy instead of degrading to plain text — proven general by an exhaustive decoder round-trip over 66,430 adversarial strings plus 20,000 seeded-random ones, not by fixture pass; `make_include_edge_key()` escapes its own `#`/`>` separators inside each docname component; `derive_master_edge_keys()` is bounded at a freshly-measured 500 frames raising a named `ExtensionError` instead of an uncaught `RecursionError`; and `_track_image()`'s absolute-URI gate moved onto a backslash-normalized platform-independent predicate with a SHA-1[:8]-prefixed relocation key over the whole URI. **A success criterion was found to contradict itself and was amended on measurement rather than silently satisfied** — ROADMAP SC#4 named a predicate literally while requiring behaviour that predicate measurably does not produce; the plan stopped at a blocking checkpoint and the owner chose the spelling — Validated in Phase 55 (55-VERIFICATION.md `passed` 5/5; 4 plans)

- ✓ The published documentation describes the registry that actually shipped — v0.9.0 Phase 56 (DOC-15..17): a two-way **AST-pinned** error catalogue for the registry (AST-based because one site raises via a shared helper and another uses implicit string concatenation), a network-free two-master worked example, a seven-case key-naming rules table bound to code **by import** rather than transcription, a new Removed Configuration Values section for the three v0.9.0-era removals, and `output_layout.rst`'s stale single-root `_template.typ` story rewritten into the real per-key bundle story with both published file counts corrected **in the same commit as the test that pins them**. The hand-compile `--root` consequence ships **conditionally** on the target's own path shape — a bare target needs no root, a target with a path component does — with both branches proven by a real `typst.compile()` gate built at runtime, after the CONTEXT-time claim was reproduced twice and found false. The stale "accepted and ignored" definition of element [4] was retracted, and the discovery grep run at execution time found one hit beyond every written floor — Validated in Phase 56 (56-VERIFICATION.md `passed` 8/8, UAT 1/1; 5 plans)

- ✓ v0.9.0 released — v0.9.0 Phase 57 (REL-08 prep half; prep-only, one owner-approved product line) + the `/gsd-complete-milestone` publish (REL-08 publish half): the tree bumped to `0.9.0` across `pyproject.toml`/`README.md`/`uv.lock`, the `## [0.9.0]` CHANGELOG section curated around **four** `**Breaking` marks with the tail link block rolled over, a `Migrating from 0.8.x to 0.9.0` guide written from a real `-b typst` build at the `v0.8.0` tag in a separately-provisioned worktree, and the post-bump tree proven green across three surfaces — CI run `32557477023` at 12/12 including both `windows-latest` lanes, a local suite of 1425 passed / 1 skipped with `black`/`ruff`/`mypy` clean and a built-wheel content check, and the multi-template PDF gate at 6 passed / 0 skips with a `pypdf` page-geometry read-back showing A4-vs-US-Letter divergence. The phase took **zero irreversible action** and held REL-08 at `[ ]` through all eleven plans, guarding it with a SHA-256 of `REQUIREMENTS.md` recorded at phase head and three separated tag/release probes. The publish then executed: PR #134 merged (`68b92e24`) with all 15 checks green, tag `v0.9.0`, release run `32560457509` completing `validate` → `build` → `publish-pypi` → `create-release` all success, PyPI `typsphinx 0.9.0` live, the Release body byte-identical to the extractor's 123-line output with all three assets, and `typsphinx-doc-translations` pinned to `68b92e24` via its own dispatched `update-pin.yml` and tagged `v0.9.0` on `f9390d4b` — Validated in Phase 57 (57-VERIFICATION.md `passed` 5/5 must-haves, 0 gaps, 0 human-verification items) + v0.9.0 close (2026-08-22)

- ✓ A Windows-shaped absolute image URI survives the whole pipeline — v0.9.1 Phase 59 (PATH-01, IMG-04, IMG-05, IMG-06, IMG-07): `_escapes_outdir()` binds `stem.replace("\\", "/")` once and feeds that single normalized string to both `posixpath.isabs()` and `_is_drive_qualified()`, matching the idiom its sibling `_is_absolute_image_uri()` already shipped, and the hardening was **proven to change nothing live** by a parametrized pin measuring both production call sites byte-identically before and after. `_track_image()`'s relocation key moved into two new module-level helpers: `_build_relocation_key()` hashes the **raw** `resolved_uri` for the digest (the collision anchor IMG-03 closed, deliberately not the normalized string) while normalizing only the basename half, and `_bound_relocation_component()` bounds the final path component to 255 UTF-8 bytes with the `{sha1[:8]}-` prefix whole and every cut on a character boundary. `visit_image()` now binds `escape_typst_string(adjusted_uri)` once and interpolates that one value at **both** emission sites. Eight new test files, **zero pre-existing test edits** — measured with `git diff --name-status` yielding only `A` lines, never claimed. **Two locked decisions were falsified by measurement and amended, both owner-approved after independent re-measurement**: `59-CONTEXT.md` D-01 and ROADMAP SC#2 both predicted the unfixed tree would be refused with `path must not contain a backslash`; it is actually refused with `unclosed delimiter`, because the unfixed pipeline emits a literal carrying a raw backslash **and** a raw unescaped `"` simultaneously, and the `"` ends the string at parse time before the semantic backslash check runs — D-01's four probe runs had each carried only one defect. The substantive claim was unaffected and measured true: all three of unfixed / key-normalization-only / escaping-only fail to compile, only the tree with both halves compiles. **The phase's own code review then found a blocker its five plans had not**: `_bound_relocation_component()` reserved one *byte* for the stem, which empties a multi-byte leading character once the UTF-8 walk-back runs — violating the "never an empty basename" property SC#3 names. Reproduced independently, fixed in-phase with the reserved unit changed to one *character* and the shortfall borrowed back from the extension, with two regression gates proven RED against the pre-fix tree first. **CI caught the other defect no local run could**: run 1 failed on `ruff` UP012 alone while all six matrix test jobs passed — ruff is unrunnable in the freshly-`uv sync`ed worktrees every executor ran in, which is precisely why CI holds lint authority here. Run 3 (`924f21d8`) is the acceptance run: 12/12 green including both `windows-latest` jobs, dispatched fresh on the phase's own post-fix tip — Validated in Phase 59 (59-VERIFICATION.md `passed` 5/5 must-haves, 0 gaps, 0 human-verification items; 5 plans in 5 sequential waves)

- ✓ A message site can move off `!r` without a single test edit — v0.9.1 Phase 58 (MSG-01): the two tests that hard-coded `repr()`'s output format as their pass criterion now assert the *meaning* — that the offending path is named in the message — through a new shared `path_named_in()` predicate in `tests/_path_naming.py`. Both rewrites were proven neither regression nor tautology by a **real, recorded RED** against a temporarily-edited `typsphinx/builder.py`, working tree restored clean afterwards. The completeness half is a self-excluding, non-vacuous AST sweep over `tests/`, re-derived at run time and locked to an explicit seven-site allowlist, which brought the whole-tree `repr()`/`!r` pass-criterion census to exactly 7 with **zero path-valued sites left** — the census file `58-REPR-CENSUS.md` then became the measurement instrument Phase 59 and Phase 60 each used to prove their own zero-test-edit claims, rather than each asserting it. Ships **no product change** — Validated in Phase 58 (58-VERIFICATION.md `passed` 5/5 must-haves, 0 gaps, 0 human-verification items; 3 plans in 3 sequential waves)

- ✓ Every user-facing message that names a path quotes it unambiguously and without backslash-doubling — v0.9.1 Phase 60 (MSG-02, MSG-03, MSG-04, MSG-05): a new **zero-import leaf module** `typsphinx/pathfmt.py` carries `quote_path()`, which reproduces `repr()`'s delimiter-selection rule (prefer `'…'`; switch to `"…"` when the value contains a single quote and no double quote; escape only when it contains both) minus the backslash doubling that made a Windows path unmatchable — gated by a 27-test RED-first suite. All 23 path-valued interpolations in `builder.py` route through it, **including two `target` sites the original enumeration missed**, closed behind a RED-first gate at all five message families plus a type-narrowing control proven green before and after; `writer.py`'s wrapper-render DEBUG log routes its three, with a two-tree byte-identity proof for the live package-alone `None` case; and `template_registry.py`'s CONF-17 violation and existence-check messages route `template`, closing both a backslash-doubling defect **and** a leaked `PosixPath(...)` wrapper defect. The adjacent type-check message stays measurably on `repr()` as SC#3's deliberate exclusion — its interpolated value is reached only when it is *not* path-shaped, so quoting it as a path would be actively wrong. The repo-wide SC#2 grep found a **fourth** module carrying the same defect shape (`translator.py`'s two relative-path DEBUG logs) and **filed it as a todo rather than fixing it in-phase**, outside MSG-02..MSG-05's scope — Validated in Phase 60 (60-VERIFICATION.md `passed` 5/5 must-haves, 0 gaps, every claim re-measured independently of the SUMMARY/EVIDENCE files; 5 plans)

- ✓ The milestone closed out without publishing, and the fence that has slipped at five consecutive prior release-prep closes held — v0.9.1 Phase 61 (REL-09 deliberately **not** satisfied, D-08): SC#1 (atomic bump to 0.9.1) was **DROPPED** outright, SC#2 **REWORDED** so the milestone's PATH/IMG/MSG bullets are authored under the existing `## [Unreleased]` heading with the tail link block untouched and no version literal moved, SC#3 **re-anchored** to the milestone-final tree (local 1513 passed / 5 skipped with `black`/`mypy`/version-sync clean; CI `workflow_dispatch` run `33260111745` 12/12 `success` including both `windows-latest` lanes), SC#4 **RETAINED in full** (two fence observations 38m16s apart spanning two waves and a full 3-OS dispatch, each network probe carrying a real positive control, the empty `typsphinx/` diff proven non-vacuous by a live widened control, and a SHA-256 guard on `.planning/REQUIREMENTS.md` re-verified with an explicit MATCH at phase close), and SC#5 **RE-AIMED** at what the *v0.9.2* milestone inherits rather than at a publish this close does not perform. `61-HANDOFF.md` opens with the negative — *this milestone publishes nothing* — before any checklist item, inverting seven consecutive prior handoffs. No tag, no PyPI upload, no GitHub Release, no PR, no translations-repo pin advance, and no public-surface disclosure of the blocker (D-05) — Validated in Phase 61 (61-VERIFICATION.md `passed` 9/9 must-haves, 0 gaps, 0 human-verification items; 4 plans)

- ✓ An image anywhere but first in its container compiles, and every master gets its PDF — v0.9.2 Phase 62 (IMG-08, IMG-09, IMG-10, TEST-05): `visit_image()`/`depart_image()` joined the separator triad the rest of the translator already runs on (`_add_paragraph_separator()`, `_emit_inline_concat_separator()`, and the `in_list_item` / `list_item_needs_separator` pair, with `_mark_inline_concat_content()` bookkeeping on departure) as a **9-line pure insertion with zero deletions** — both the `in_figure` and `else` branch **bodies** stay textually unmodified and no new line-boundary predicate was introduced, which a repo-wide grep for `endswith("\n")` / `rstrip().endswith` / `[-1:]` over `translator.py` re-confirms returns nothing. **The mechanism was AMENDED on a live measurement before a line was written**: driving the triad from the non-`in_figure` branch alone, as the requirement first specified, was measured to leave **4 of 18 masters still refused** — both legend shapes (a legend image has `self.in_figure == True` and never reaches that branch), the field-list-body concat shape (where the unconditional trailing newlines break the concat with a *new* refusal, `cannot apply unary '+' to content`), and `index` transitively — so the leading half was hoisted above the `if self.in_figure:` split and the trailing half made concat-aware. That delivers strictly more of the requirement, never less. The acceptance gate is a real `typst.compile()` driven through `-b typstpdf` over a 27-document / 18-master fixture from **one** build invocation, binding all **16** measured failing shapes and all **9** that must keep passing, and it was recorded RED first: `typsphinx/translator.py` restored from the phase base SHA, the 17-master aggregate `ExtensionError` (`expected semicolon or line break`) transcribed verbatim with a positive control, the fix restored, `git status --porcelain` recorded empty. Eight of the nine PASS shapes are bound byte-identical to committed goldens; the ninth gains exactly one empty line and is pinned by an **exact-delta** assertion rather than waived to "compiles". **Zero pre-existing test edits**, measured with `git diff --name-status` over the 20 files carrying the 144 `image(` matches, not asserted — Validated in Phase 62 (62-VERIFICATION.md `passed` 4/4; 4 plans)

- ✓ 0.9.2 released to PyPI, carrying v0.9.1's unpublished work with it — v0.9.2 Phase 63 (REL-09 prep half, REL-10, REL-11; prep-only, zero irreversible action) + the `/gsd-complete-milestone` publish (REL-09 publish half): the tree bumped to `0.9.2` in one commit across `pyproject.toml`, a regenerated `uv.lock` (which carries its own self-package version literal — omitting it reproduces the `uv sync --locked` failure class already killing every dependabot PR across eleven CI steps), `README.md` and `CHANGELOG.md`; the `### Planned for Future Releases` scratch block relocated beneath a **fresh empty** `## [Unreleased]` heading *before* the old heading became `## [0.9.2]`, because `scripts/extract_changelog_section.py` selects by **position** rather than by heading name and renaming in place would have carried the block into the published release notes (REL-10, closed by running the script and reading its stdout, not by reasoning the edit was correct); and the bumped tree proven green on runs executed in the phase — 1543 passed / 5 skipped, `black --check .` and `mypy typsphinx/` exit 0, two clean documentation builds from a removed `docs/_build`, and CI run `33309565005` at 12/12 including both `windows-latest` and both `macos-latest` lanes with `ruff`'s verdict read from the `Lint and Format Check` job's own step. **A post-plan gate then caught a false blanket claim inside the curated section itself** — "the runtime changes are confined to `typsphinx/translator.py`, with no other file under `typsphinx/` touched", falsified by that same evidence file's own invariant sweep showing five files changed since 0.9.0 — found independently by `63-REVIEW.md` CR-01 and `63-VERIFICATION.md`'s SC#2 block *after* the extractor's structural checks had passed clean; the sentence was deleted, a narrower measured version re-scoped into the one bullet it is true for, and the byte-identity proof re-taken against the corrected text. The publish then executed: PR #136 merged (`45962faa`) with 15/15 checks green, tag `v0.9.2`, release run `33318905691` completing `validate` → `build` → `publish-pypi` → `create-release` all success, PyPI `typsphinx 0.9.2` live with wheel and sdist, the Release body byte-identical to the extractor's 4083-byte / 54-line output with zero scratch-block leakage, and `typsphinx-doc-translations` pinned to `45962faa` via its own dispatched `update-pin.yml` (`fcf66da4`) and tagged `v0.9.2` — Validated in Phase 63 (63-VERIFICATION.md `passed` 5/5, UAT 25/25, 0 gaps) + the v0.9.2 close (2026-08-31)

- ✓ The release-checkbox fence held a second consecutive time, on a requirement that was meant to close — v0.9.2 Phase 63 (REL-11): a SHA-256 of `.planning/REQUIREMENTS.md` recorded at phase head with a labelled `PHASE_BASE_SHA` anchor, re-verified at phase close, again after the gap-closure commits, once more inside `63-HANDOFF.md`, and a final time by the operator at the milestone close **after** all `phase.complete`-family tooling had run — MATCH on the digest, the line count, the empty `git diff --name-only` and the three `REL-09` grep hits (lines 70/154/175), every observation. Every plan in the phase declared `requirements-completed: []` for REL-09, closing the subtler hazard the v0.9.1 audit found, where three of Phase 61's four plans declared the opposite of their own correctly-unmet checkbox. `phase.complete` has auto-flipped the release requirement against an explicit CONTEXT decision at **five consecutive** prior release-prep closes; it did not flip here — Validated in Phase 63 (63-CLOSEOUT-GUARD.md, three recorded observations + the operator's fourth at the close)

- ✓ Every documented bare command and every tox environment runs on the maintainer's NixOS machine — v0.9.3 Phase 64 (NIX-01..NIX-08): `flake.nix` gained one Linux-guarded `buildFHSEnv` passthrough (`typsphinx-fhs-run`, a pure `exec "$@"`) and seven command shims (`uv`, `tox`, `ruff`, `black`, `mypy`, `pytest`, `sphinx-build`) that walk up to the nearest `.venv` and exec its binary by absolute path inside the sandbox; both darwin systems evaluate byte-identically and stay unverified by construction. The first genuine-shape measurement (64-02) found `tox -e py312`, `tox -e cov` and the full suite failing on `libz.so.1`: Pillow's `_imaging` carries a bare `NEEDED libz.so.1` and uv-managed CPython links zlib statically, while `tox -e py313` passed only because the nix interpreter's own `zlib` module had already mapped libz — an import-order accident, not a healthy sandbox. Gap closure added `targetPkgs = p: [ p.zlib ]` (64-05, gated by a DT_NEEDED residual audit before and after) and re-measured everything in a Claude Code session relaunched after the edit (64-06): `ruff 0.15.20`, 1543 passed / 5 skipped, all seven tox environments OK from cold, a real `%PDF` from docs-pdf, and no unresolved `libz.so.1` in anything provisioned — Validated in Phase 64 (64-VERIFICATION.md `passed` 9/9 on re-verification after gaps_found 6/9; 6 plans in 5 waves, two session relaunches)

- ✓ The repository is back on the upstream `tox-uv` package, and which `uv` a real tox run resolves is observed rather than assumed — v0.9.3 Phase 65 (TOX-01..TOX-04): one four-file commit (`d32eb5db`) swapped `pyproject.toml`'s `dev` extra to `tox-uv>=1.35,<2` and `tox.ini` to the comma-free `requires = tox-uv~=1.35`, carried a `uv.lock` regenerated by a plain `uv lock` (`tox-uv 1.36.0`, `tox-uv-bare 1.35.2 -> 1.36.0`, `uv 0.12.13`), and inverted the Phase 45.2 static gate rather than deleting it (D-06 AMENDED). Inside FHS a cold `tox -vv -e py312 -r` logged tox-uv's bundled branch resolving the worktree's own `.venv/bin/uv` at exactly the locked `uv 0.12.13` (1543 passed / 5 skipped), with no `TOX_UV_PATH` anywhere; an isolated control on a nix-interpreter venv outside FHS failed on that same `uv venv` exec (exit 127) and nothing earlier. CI run `34681968010` on the pushed tip `d9c75553` completed 12/12 `success`, and every test lane plus lint now resolves the runner's `.venv` uv instead of setup-uv's `hostedtoolcache` uv — Validated in Phase 65
...
- ✓ Sphinx's own link check runs over `docs/source/` with one command, and every surface listing the tox environments names it — v0.9.5 Phase 72 (QUA-13, DOC-24): `tox.ini`'s new `[testenv:linkcheck]` is shaped exactly like `docs-html` and sits outside `env_list`, so plain `tox` does not run it. A fresh run reports **95 links, 95 `working`**, zero non-transient and zero transient failures, exit 0, with no `linkcheck_*` key added to `conf.py`. `CLAUDE.md`, `README.md` and `docs/source/contributing.rst` — the three surfaces a fresh repo-wide grep found — all list it, and the 2026-07-22 linkcheck-CI todo carries a D-07 status note and stays open in `todos/pending/` (QUA-08 deferred) — Validated in Phase 72 (72-VERIFICATION.md `passed` 5/5, UAT 1/1, SECURITY 26/26 closed; 6 plans)

- ✓ The HTML sidebar shows each User Guide and Examples page once, and the Typst output still includes each page exactly once — v0.9.5 Phase 72 (DOC-18): the root `docs/source/index.rst` toctrees list only `user_guide/index` and `examples/index`, dropping the five duplicated children. A clean build's `document is referenced in multiple toctrees` messages went **5 → 0** (`MULTI_TOCTREE_BASE = 5`, `MULTI_TOCTREE_TIP = 0`) with the counted warning totals unchanged (docs-html 3 → 3, docs-pdf 5 → 5) and every page still included exactly once under `-b typst`. The narrower HTML-vs-Typst `examples/basic` parent divergence was re-measured at the phase tip and did **not** survive (`DIVERGENCE_SURVIVES = no`), so no todo was filed — Validated in Phase 72 (72-VERIFICATION.md `passed` 5/5; 6 plans)

- ✓ The milestone merged to `main` with nothing published — v0.9.5 Phase 73 (prep-only) + the `/gsd-complete-milestone` merge (REL-14): two CHANGELOG bullets under `## [Unreleased]` (`### Added` for the linkcheck environment, `### Fixed` for the sidebar dedup) as a pure addition — 15 added lines, 0 removed, headings 23 → 23, no version literal moved — the tree green locally (1547 passed / 1 skipped, twice, once under `LC_ALL=C`; ruff/black/mypy clean; both docs environments clean-built; `tox -e linkcheck` 95/95) and in CI (run `35083828156`, 12/12 including both `windows-latest` and both `macos-latest` lanes), and the REQUIREMENTS.md SHA-256 fence matching at every observation — the auto-flip fired at `phase.complete` and was reverted before commit; then PR #151 merged as `43fd7c13` and REL-14 was checked on five observations (merge commit on `origin/main`, `pyproject.toml` `0.9.2`, no `v0.9.5` tag with `v0.9.2` as positive control, PyPI 404 for `0.9.5` and 200 for `0.9.2`, no `v0.9.5` Release) — Validated in Phase 73 (73-VERIFICATION.md `passed` 4/4; 7 plans) + the v0.9.5 close (2026-09-16)
...
- **WR-03** — the "Custom template not found" warning fires three times instead of two for one narrow
  shape (54.1 WR-01).
- **QUA-08** — a weekly advisory CI workflow calling `tox -e linkcheck`. Scoped into v0.9.5 and
  deferred to Future at roadmap review (2026-09-13): GitHub runs `schedule` only on the default
  branch and `workflow_dispatch` only for a workflow file already there, so it could not be proven
  on the unmerged milestone branch. The environment it would call now exists on `main`, so the
  obstacle is gone.
...
- [x] `tox -e linkcheck` runs Sphinx's `linkcheck` over `docs/source/` and is listed with the
      other tox environments (QUA-13, DOC-24). The weekly CI job (QUA-08) was deferred to Future at
      roadmap review — Phase 72.
- [x] The root `index.rst` toctrees list only section indexes, so a clean HTML build emits zero
      `referenced in multiple toctrees` messages and the sidebar shows each page once (DOC-18) — Phase 72.
- [x] Close prep, unpublished: CHANGELOG bullets under `## [Unreleased]`, `pyproject.toml` stays
      `0.9.2`, no tag / PyPI / Release.
      Merged to `main` via PR #151 (REL-14) — Phase 73 + the close.
...
- [x] `tox-uv-bare` → `tox-uv` reverted in `pyproject.toml` and `tox.ini`.
- [x] `.github/dependabot.yml` switched from `package-ecosystem: "pip"` to `"uv"`, closing
...
**v0.9.2 Inline image blocker fix and release (SHIPPED 2026-08-31)** — close the blocker that
stopped v0.9.1 from being published, then publish v0.9.1's completed work and this fix together as
0.9.2:

- [x] An image node preceded by sibling content in the same paragraph or list item is emitted with a
      separator before `image(`, so the document compiles instead of aborting the whole build with
      `expected semicolon or line break` — the `in_figure` indented branch stays unseparated —
      **Phase 62** (IMG-08). Delivered as an AMENDED mechanism: the leading separator triad is
      hoisted *above* the `if self.in_figure:`/`else:` split rather than added inside the
      non-`in_figure` branch, because the branch-scoped form was measured to leave 4 of 18 masters
      red. The commitment holds as written — both `in_figure` branch **bodies** are byte-identical
      to the phase base SHA, verified independently; the net change is a 9-line pure insertion.
- [x] A real `typst.compile()` gate binds all four measured failing shapes (substitution image
      mid-sentence, two images in a row, an image inside a list item, an image preceded by any
      sibling content) and the two shapes that must keep passing (image first in its paragraph,
      image inside `.. figure::`), RED-first against the unfixed tree —
      **Phase 62** (IMG-09, TEST-05). Delivered as a **superset**: the gate binds all **16**
      measured failing shapes (one master each) and all **9** must-keep-passing shapes, and the
      RED was reproduced against a genuinely restored unfixed tree — `expected semicolon or line
      break` on 17 masters — then transcribed verbatim into `62-RED-EVIDENCE.md` with a positive
      control. The 9 passing shapes are pinned to committed goldens; the one non-byte-identical
      shape is pinned to an exact committed delta rather than waived.
- [x] `-b typstpdf` produces a PDF for every master of a project containing a mid-paragraph inline
      image, where today it raises `ExtensionError` and writes no PDF for any master —
      **Phase 62** (IMG-08, IMG-09). Measured on the 18-master / 27-document fixture: 18/18 masters
      write a non-empty `%PDF` file from one `sphinx-build -b typstpdf`, including the image-free
      `index` master that failed only because Typst's `#include()` re-parses a poisoned content file.
- [x] 0.9.2 released to PyPI (REL-09, carried from v0.9.1) with one curated `## [0.9.2]` CHANGELOG
      entry covering v0.9.1's accumulated `## [Unreleased]` bullets plus this fix, no `## [0.9.1]`
      heading, the version bumped as the sole literal in `pyproject.toml` with `uv.lock` and
      `README.md` in lockstep, and a GitHub Release body sourced from
      `scripts/extract_changelog_section.py` — **Phase 63 prep half + the `/gsd-complete-milestone`
      publish** (REL-09, REL-10, REL-11). PyPI `typsphinx 0.9.2` live from release run
      `33318905691`; the Release body measured byte-identical to the extractor's 4083-byte output
      with zero scratch-block leakage.

</details>

<details>
<summary>v0.9.1's Active list (product side complete, release cancelled 2026-08-30) — retained for reference</summary>

**v0.9.1 Windows path correctness (COMPLETE 2026-08-30, deliberately NOT published)** — the Windows
path defects held behind Phase 57's prep-only fence, closed together with their whole sibling
family. Every headline commitment below is met; the milestone was withheld from release for a
*different*, pre-existing blocker (see Current State):

- [x] `_escapes_outdir()` applies its absolute-path and drive-qualified checks to the
      backslash-normalized string, not the raw stem, so a driveless-absolute Windows-shaped
      `typst_documents` target is refused by OUT-02's guard — **Phase 59** (PATH-01)
- [x] `_track_image()`'s escape branch is safe end to end: the relocation key is built from a
      ...
      kept as the collision anchor — **Phase 59** (IMG-04, IMG-05, IMG-06, IMG-07)
- [x] One delimiter-aware path-quoting helper carries every path-valued interpolation in
      ...
      `'...'` delimiter dropped — **Phase 60** (MSG-02..MSG-05; `typsphinx/pathfmt.py::quote_path()`,
      23 sites in `builder.py` including two the enumeration missed, 3 in `writer.py`, 2 in
      `template_registry.py`; a fourth module found by the repo-wide grep and filed as a todo)
- [x] Every fix carries a gate that is RED against the unfixed tree first, at least one of them a
      real `typst.compile()`, with POSIX output proven byte-identical by zero test edits —
      **Phases 58, 59, 60** (the compile gate is `test_windows_image_uri_render_gate.py`; the
      zero-test-edit claim is measured against `58-REPR-CENSUS.md` and `git diff --name-status`,
      not asserted)
- [x] The 3-OS CI lane, `windows-latest` included, green over the fix — **Phases 59 and 61**
      (run `924f21d8`/12-of-12 at the Phase 59 acceptance tip; run `33260111745` 12/12 at the
      milestone-final tree)
- [ ] v0.9.1 released to PyPI (REL-09) — **deliberately unmet (D-08)**. The release was cancelled
      for the inline-image blocker; the requirement carries to v0.9.2 with its obligations unchanged
      and its version token corrected to 0.9.2.
...
**v0.9.0 per-document templates (SHIPPED 2026-08-22)** — every `typst_documents` entry can use its
own template, package, and template-function arguments:

- [x] A new `typst_document_templates` registry names template definitions (`template` xor
      `package`, plus `template_function`), and `typst_documents` element [4] selects one — the slot
      `configuration.rst:80` used to define as "accepted and ignored" — **Phases 53 and 54**
- [x] The built-in key `"typst"`, and a four-element tuple, defer to global config, so every
      existing `conf.py` keeps working unchanged — **Phase 53**, proven byte-identical against a
      pre-change SHA-256/page-count baseline across all four existing configuration shapes
- [x] Every key's template bundle — `"typst"` included, with no exception — is copied wholesale to
      `<outdir>/_template/<key>/`, so template-relative asset references resolve — **Phase 54**
- [x] `_write_template_file()`, `_copy_template_directory()`'s `.typ` exclusion,
      `copy_template_assets()`'s three early returns, and the reserved `_template.typ` file claim
      are deleted rather than extended; `typst_template_assets` is removed as inert config, with a
      `config-inited` warning shim naming the value and its consequence — **Phase 54**
- [x] Misconfiguration fails loudly **before any `.typ` file is written**: unregistered key,
      `template` + `package` in one entry, a user-defined `"typst"` key, a `template` escaping its
      bundle, and a bundle directory colliding with Sphinx's `templates_path` — **Phases 53 and
      54.1** (the pre-write half was the inserted phase's own work)
- [x] The five v0.8.0-derived defects that shipped unfixed by decision D-01, or with only a
      test-side fix, are closed on the product side — **Phase 55**

**Shipped with known limitations**, deferred by owner decision and recorded in `57-HANDOFF.md`
§ "Deferrals carried forward": the `templates_path` collision check resolves against `srcdir` rather
than `confdir` so `-c`/confdir projects are uncovered (WR-02, shipped silent — third consecutive
release to decline a `### Known Limitations` section); the "Custom template not found" warning fires
three times instead of two for one narrow shape (54.1 WR-01); and the pre-write refusal messages
quote paths with a fixed `'...'` delimiter, so a path containing a literal single quote closes the
quote early (57 WR-01).
...
- **`ruff` cannot run locally on NixOS** — the same generic-linux-ELF family as QUA-04, but on the
  ...
  cannot run one of the three gates CI enforces. **→ Scoped into v0.9.3, together with its `tox -e
  py312` companion defect; no longer carried forward.**
...
- **Completed-but-unshipped state (2026-08-30, v0.9.1):** PyPI still carries `typsphinx 0.9.0`. There is no `v0.9.1` tag and no GitHub Release. v0.9.1's product work **is** on `main` — merged as `9db2274c` via merge-only PR #135 with 15/15 checks green, at owner instruction after the close — but `pyproject.toml` is still at `0.9.0` and the milestone's CHANGELOG bullets are still under `## [Unreleased]`, so nothing is released.  The merge exists so 0.9.2 can carry the fixes. Four bundled `@preview` packages unchanged. **Zero runtime dependencies added since v0.6.0 and no new `typst_*` config value** — this was a bug-fix round by construction, and the one new module (`typsphinx/pathfmt.py`) is a zero-import stdlib leaf. Read the Docs `en`/`ja` still serve 0.9.0.
- **Open defects the project knows about (as of the v0.9.1 close, 2026-08-30):** the headline one is the **inline-image blocker** — an image not first in its paragraph aborts the whole Typst compile, so `-b typstpdf` produces no PDF at all. It is live in the *published* 0.9.0, undisclosed on every public surface by D-05, and is the reason v0.9.1 was withheld. Alongside it: the `translator.py` hardcoded-delimiter DEBUG logs filed by Phase 60's own grep (the fourth module of the MSG-02 family); WR-02's `templates_path`-resolves-against-`srcdir` gap and the tripled "Custom template not found" warning, both carried unchanged from v0.9.0; `:numref:` per-master divergence, still excluded from every published surface by D-07 (v0.8.0); the dependabot `uv.lock --locked` breakage, which still kills every dependabot PR before a single test runs; the root-toctree HTML sidebar duplication in this project's own docs; and `ruff` still unrunnable in a freshly-provisioned venv on NixOS. *(Superseded: the two `builder.py` path-predicate todos filed 2026-08-16 and the `repr`-escaped-paths todo — both halves — were closed on the product side by Phases 59 and 60.)*

- **Shipped state (2026-08-22, v0.9.0):** PyPI `typsphinx 0.9.0` — Sphinx 9.1 / docutils 0.22 / typst 0.15, Python 3.12–3.13. Four bundled `@preview` packages, unchanged through this milestone and guarded across four sync surfaces; **zero runtime dependencies added since v0.6.0**, argued at hunk level over the SHA-anchored milestone diff with the detector fire-tested against a real positive control. Phase 52's "no new `typst_*` config value" assertion **does not carry over** — this milestone removed one (`typst_template_assets`) and added one (`typst_document_templates`). Documentation on Read the Docs in English and Japanese, the latter built from `typsphinx-doc-translations`, which carries its own matching `v0.9.0` tag on pin `68b92e24`. **The output shape changed in a user-visible, breaking way for the second consecutive minor release** — the template file moved from a single `<outdir>/_template.typ` to a per-key bundle directory `<outdir>/_template/<key>/` whose whole contents are copied, and `typst_template_assets` is gone.
- **Open defects the project knows about and chose to ship (as of the v0.9.0 close, 2026-08-22):** three, all recorded only in `57-HANDOFF.md` and `.planning/todos/pending/` — the `templates_path` collision check resolving against `srcdir` rather than `confdir` (WR-02, shipping *silent* against the reviewer's own recommended minimum of a CHANGELOG carve-out, so the shipped "validated before anything is written" sentence reads unconditional); the tripled "Custom template not found" warning for a synthesized `"typst"` key naming a nonexistent path; and the fixed-`'...'`-delimiter path quoting in the pre-write refusal messages, where a path containing a literal single quote closes the quote early. Carried forward unresolved from earlier milestones: `:numref:` per-master divergence (excluded from every published surface by D-07), the two `builder.py` path-predicate todos filed 2026-08-16, the dependabot `uv.lock --locked` breakage, the root-toctree HTML sidebar duplication in this project's own docs, and `ruff` still unrunnable in a freshly-provisioned venv on NixOS — the last **re-annotated with a live 2026-08-22 recurrence** that falsified this milestone's own 2026-08-16 measurement. *(Superseded: the four v0.8.0 defects shipped unfixed by D-01, plus the `_track_image()` `isabs()` gap, were all closed on the product side by Phase 55.)*
...
| **Keep `tox -e linkcheck` outside `env_list`** (Phase 72, 2026-09-13) | An environment that reaches the network on every plain `tox` run makes the default task runner fail on a third party's outage | ✓ Good — `tox` is unchanged for everyone; the check is one explicit command, green at 95/95 on a clean run with no `conf.py` timing key. The accepted cost is carried as LNK-01: no failure-tolerance override, so a flaky URL fails the env locally with no guidance |
| **Hold REL-14 at `[ ]` behind a SHA-256 fence and check it only at `/gsd-complete-milestone`, on the observed merge** (Phase 73, constraint 10) | The auto-flip had landed at 8 of the 9 prior release-prep closes; the one non-firing close was treated as an outlier, not a fix | ✓ Good — it fired again at Phase 73's `phase.complete` (9 of 10), was caught by the fence and reverted before commit, and REL-14 was checked only after PR #151 merged, on five positively-controlled observations |
...
| **Publish 0.9.2 covering both milestones under one heading, and never create a `## [0.9.1]` heading or tag** (owner, carried from D-02/D-08 with the version token corrected 2026-08-30) | v0.9.1 was cancelled, not skipped; a retroactive heading for a release that does not exist would be a fiction in the published record. REL-09's obligations were unchanged, only its version token | ✓ Good — one curated `## [0.9.2]` section covers PATH-01, IMG-04..IMG-07, MSG-01..MSG-05 and this milestone's fix. `git tag -l 'v0.9.1'` is empty locally and on the remote, and the published CHANGELOG has no `[0.9.1]` heading or tail link |
| **Relocate the `### Planned for Future Releases` scratch block beneath a fresh empty `## [Unreleased]` before renaming the old heading, and close REL-10 by reading the extractor's stdout rather than by reasoning the edit was correct** (v0.9.2 roadmap constraint 7) | `scripts/extract_changelog_section.py` selects by **position** — first `## [<version>]` line through the next `## [...]` of any name — so renaming in place would have carried the scratch block verbatim into the published GitHub Release body | ✓ Good — measured twice: at phase time on the file, and again against the *published* body, where `grep -c 'Planned for Future Releases'` returns `0` |
| **Check REL-09/REL-10/REL-11 at the milestone close, on the observed publish, and hold them at `[ ]` through every plan** (v0.9.2 roadmap constraints 8 and 9) | The prep-only convention, held for nine consecutive milestones. `phase.complete` has auto-flipped the release requirement against an explicit CONTEXT decision at five consecutive prior release-prep closes, and the checksum guard is the only measure that has ever stopped it. The v0.9.1 audit also found three of Phase 61's four plans declaring `requirements-completed: [REL-09]` against their own correctly-unmet checkbox | ✓ Good — the fence held at four separated observations including one taken **after** all `phase.complete`-family tooling had run, every plan declared `requirements-completed: []` for REL-09, and the boxes were checked by the operator only once PyPI carried `0.9.2` |
| **Correct a false claim inside the curated release notes and re-take the byte-identity proof, rather than shipping text that had already passed the structural checks** (Phase 63 gap closure, `63-05`) | The `## [0.9.2]` intro asserted the runtime changes were confined to `typsphinx/translator.py`; the same evidence file's own invariant sweep showed five files changed since 0.9.0. The extractor had been run and its output read, and the structural set had come back clean — structural correctness is not the same property as the prose being true | ✓ Good — found independently by `63-REVIEW.md` CR-01 (Critical) and `63-VERIFICATION.md`'s SC#2 block after all four plans had reported complete. The blanket sentence was deleted, a narrower measured version re-scoped into the one bullet it is true for, and the corrected text is what shipped |
| **Close v0.9.2 without a `MILESTONE-AUDIT.md`** (2026-08-31) | Both phases carried `phase_complete: true` / `verification_status: passed`, UAT closed 25/25 with zero issues, the pre-close artifact audit reported no open items, and 7/7 v1 requirements were complete before the close began. A two-phase milestone with a real-compile acceptance gate has little cross-phase integration surface for an audit to find | — Pending. This is the second time in three closes that the audit was skipped after v0.9.1 produced one and it demonstrably earned its place. Recorded as an absence in MILESTONES.md rather than claimed as a pass; if a gap surfaces post-release, revisit whether the audit should be mandatory regardless of phase count |
| **Cancel the v0.9.1 release outright and skip the version number — the next published release is 0.9.2** (owner, 61-CONTEXT D-02, 2026-08-29) | An owner report on 2026-08-29 surfaced a `blocker`: an image not first in its paragraph makes Typst refuse the file, so `-b typstpdf` writes no PDF for any master. Publishing a release that cannot compile a document containing an inline substitution image was declined. The number is skipped rather than held, so no `v0.9.1` tag will ever exist | ✓ Good — the first close in this project's history to publish nothing. The no-release constraint was honoured completely and verified live at four separated observations: no tag, no PyPI upload, no GitHub Release, no translations-repo pin advance. The one part later relaxed was D-12's "no pull request": the owner instructed a **merge-only** PR (#135, merged `9db2274c`, 15/15 green) after the close, so `main` carries the fixes for 0.9.2. That relaxes how the code travels, not whether it ships |
| **Do not fix the blocker in this milestone; it belongs to v0.9.2** (owner, D-07) | Phase 61 was prep-only and `translator.py` was outside both the fence and the requirement set. Inserting a repair phase would have re-opened a closed milestone's scope after its verification artifacts were recorded | ✓ Good — v0.9.2 picked it up as its first requirement, fixed it in Phase 62 and shipped it in `0.9.2` on 2026-08-31. The todo is now in `todos/completed/`, closed against the full 16-shape matrix rather than the four it was filed with |
| **Measure whether the blocker is a regression rather than assume it** (D-06) | The whole cancel-or-ship argument turns on whether this milestone's own work caused it | ✓ Good — `git diff v0.9.0..HEAD -- typsphinx/translator.py` is 25 lines, all IMG-05's `escape_typst_string()` call; `visit_image()`'s missing separator is byte-identical to the `v0.9.0` tag. Pre-existing, so cancelling cost nothing that shipping would have preserved |
| **Give the blocker no public-surface disclosure** (owner, D-05) | No `README.md` Known Limitations entry, no CHANGELOG `### Known Limitations` section, no GitHub issue. Consistent with the v0.7.1 D-27 and v0.8.0 D-01/D-03 precedents | ✓ **Discharged by shipping, not by an erratum.** The silence lasted two days: `0.9.2`'s release notes open by naming the failure and telling 0.9.0 users to upgrade, which is the first public-surface disclosure this defect ever had. The broader pattern is unresolved — v0.9.2 is the **fifth consecutive** cycle to decline a `### Known Limitations` section, and WR-02, WR-03 and NUM-01 still ship silent |
| **REL-09 carries forward unmet, with its literal wording — including its `v0.9.1` version string — unchanged** (owner, D-08) | The owner declined both rewriting it to say `v0.9.2` and closing it as superseded. The leftover inconsistency is a version number inside a requirement that was never satisfied, which is accurate: nothing was released | ✓ Good — the milestone audit classified it `deferred`, not `unsatisfied`, and `fail_gate` did not trigger. A checked box would have been the defect |
| **Write the milestone's CHANGELOG content under the existing `## [Unreleased]` heading and leave the tail link block untouched** (D-03, D-04) | With no version bump there is no `## [0.9.1]` section for `scripts/extract_changelog_section.py` to extract. The bullets are promoted into v0.9.2's own versioned section by the same mechanism Phase 57's D-02 used across the v0.8.0 → v0.9.0 boundary | ✓ Good — the one standing release-prep step deliberately *not* performed, recorded as such rather than skipped silently |
| **Amend a locked decision when measurement falsifies it, rather than reading the criterion to fit** (Phase 59, D-01a, owner-approved 2026-08-29) | `59-CONTEXT.md` D-01 and ROADMAP SC#2 both predicted the unfixed tree would be refused with `path must not contain a backslash`; it is actually refused with `unclosed delimiter`, because the unfixed pipeline emits a raw backslash **and** a raw unescaped `"` at once and the quote ends the string before the semantic check runs. D-01's four probe runs had each carried only one defect | ✓ Good — re-measured independently, put to the owner, closed with an `AMENDED` block. The substantive claim was unaffected: only the tree with *both* halves fixed compiles. Second consecutive milestone in which a success criterion was amended on evidence |
| **File the fourth module's MSG-02 defect as a todo instead of fixing it in-phase** (Phase 60, plan 60-05) | `translator.py`'s two relative-path DEBUG logs carry the same hardcoded-`'...'`-delimiter shape Phase 60 closed in three modules, but a fourth module is outside MSG-02..MSG-05's requirement scope | ✓ Good — the repo-wide grep was run as a *discovery* step rather than a confirmation of the known list, which is what surfaced it at all. `severity: minor`; the one-line fix is `quote_path()`, which now exists |
| **Produce a `MILESTONE-AUDIT.md` for this close** (2026-08-30) | The prior six closes were all `override_closeout` with no audit — a pattern `57-HANDOFF.md` and STATE.md both flagged as having outlived being a default | ✓ Good — `v0.9.1-MILESTONE-AUDIT.md` reports requirements 10/11, phases 4/4, integration 11/11, flows 1/1, `status: tech_debt`, `fail_gate.triggered: false`. It is what let REL-09 be classified `deferred` rather than `unsatisfied` on stated grounds |
| Derive the `repr()`/`!r` census from a whole-tree `ast` sweep of `tests/**/*.py`, re-derived at run time and locked to an explicit seven-site allowlist — never from the two sites MSG-01 names, and never from a hardcoded count (D-08/D-09, 2026-08-27) | The two named sites are what the requirement already knew about; an enumeration derived from them could only ever confirm itself, and Phases 59 and 60 need to check their zero-test-edit claim against something that can go RED when it goes stale. A count constant would also have had to pick between two divergent prior total measurements (341 vs 352) that turned out to be measuring different things | ✓ Good — the sweep found nine phase-base pass-criterion sites, three more than the requirement named, including the `TestWindowsPathEscapingRegressionGuard` third bucket that is path-valued and format-asserting **by design** and must NOT be rewritten in Phase 60. The guard walks each `ast.Assert`'s `.test` only, never `.msg`; both properties were proven by live falsification (a fake pass-criterion site turned it RED, a `repr` placed only in `.msg` correctly left it GREEN). No total-occurrence figure is asserted anywhere |
| Insert Phase 54.1 to close the two Phase 54 code-review findings immediately rather than deferring them (2026-08-16) | Both were created by Phase 54's own wholesale-copy rule; deferring would have shipped a hole that republishes a project's Sphinx `templates_path` Jinja directory into public build output, while the published docs actively recommended that exact layout | ✓ Good — 5 plans, both closed pre-write, and the aggregate-message work it forced is now the shape every pre-write refusal uses |
| Ship WR-02 silent — no CHANGELOG carve-out, no `### Known Limitations` section, no issue (D-09, owner decision 2026-08-16) | The v0.7.1 D-27 and v0.8.0 D-01 precedents; the reviewer's own recommended minimum was exactly the carve-out sentence being declined, and that counter-case was put on the table before declining | — Pending, and **weaker than the prior two**: the shipped `## [0.9.0]` sentence ("template layout is now validated before anything is written") reads as unconditional, so silence here ships an over-broad true-sounding claim rather than a true claim with a missing footnote. Third consecutive release to decline a limitations section |
| Break the prep-only fence exactly once, for `57-11`'s Windows `!r`-escaping fix, recorded as a dated AMENDED block naming SC#4, `57-08` and the phase verifier as its readers (owner decision 2026-08-22) | Two full CI matrices had already been burned on the defect; a test-side-only fix was not available because the defect was in the message the test asserts against. The fence's readers were named explicitly so downstream fence checks would read the amended rule instead of reporting a false violation | ✓ Good — the fence held everywhere else (`git diff --name-only 78bd595d..HEAD -- typsphinx/` lists only `builder.py`), the downstream checks read the amendment, and the next dispatch returned 12/12 |
| Keep `ruff-generic-linux-elf-unrunnable-on-nixos` **open** rather than close it on the 2026-08-16 measurement that `ruff` works here (owner decision 2026-08-16) | No commit explained the repair, so the evidence showed the symptom absent without explaining why | ✓ Good, and vindicated within a week — the failure reproduced live on 2026-08-22 in a freshly-provisioned worktree venv. The main tree's stale patchelf'd binary masks it, so the main-tree measurement can never detect the hazard |
| Read `## [Unreleased]`'s existing seven bullets as material to **promote substantially as written**, and D-01's "exactly two `**Breaking:**` marks" as a **floor on the missing pair rather than a cap** (D-01/D-02, 2026-08-16) | Unlike the v0.8.0 precedent, `## [Unreleased]` was not empty — Phases 54/54.1/55 had already written real v0.9.0 bullets — and the two changes SC#2 names were absent from the CHANGELOG entirely while two *different* Breaking bullets were already present | ✓ Good — the shipped section carries four `**Breaking` marks and the census is pinned by a test, so neither compression nor re-authoring was needed |
| Close v0.9.0 as `override_closeout` without a `MILESTONE-AUDIT.md` (owner decision, 2026-08-22) | `init.manager` reported all 6 phases `phase_complete=true` / `verification_status=passed`, and every v1 requirement except the publish-gated REL-08 was already Complete before the close began | — Pending, **sixth consecutive close taken this way**. The pattern has outlived being a default; it deserves an explicit decision about whether the audit step still earns its place in this project's close |
...
| Repair the v0.7.0 GitHub Release by hand and revert REL-04 to Pending, rather than moving the published tag to re-run the workflow (owner decision, 2026-08-04) | Moving a tag that PyPI has already published against risks a duplicate-upload failure and rewrites published history; repairing the artifact makes the release correct for users today, while leaving REL-04 open keeps the record honest about what the automation has actually done | ✓ **Good — closed 2026-08-31.** `create-release` completed `success` on the real `v0.9.2` tag push (run `33318905691`), its third consecutive green after v0.8.0 and v0.9.0, and the published body was measured byte-identical to `scripts/extract_changelog_section.py 0.9.2` with the wheel and sdist attached. Keeping the row open for four milestones rather than closing it on the workflow file being correct is what made the eventual close mean something |
...
*Last updated: 2026-09-14 — **Phase 72 (`tox -e linkcheck` and Root Toctree Deduplication) complete and verified 5/5**, 6 plans in 4 waves in isolated worktrees, UAT 1/1 (the owner's look at the rendered sidebar), Nyquist validated, SECURITY 26/26 closed. QUA-13, DOC-24 and DOC-18 complete; code review 0 critical / 0 warning / 1 info (a suggestion to mention transient failures in the linkcheck `description`, left for the owner). The branch is on `origin` for the first time, with CI run `34761445288` 12/12 green. Next: Phase 73 (v0.9.5 close prep, unpublished). Prior footer retained below.*
...
*Last updated: 2026-09-12 — **Phase 64 (FHS Wrapper and Command Shims in `flake.nix`) complete and verified 9/9** on re-verification (the first pass read gaps_found 6/9 on `libz.so.1`), 6 plans across 5 waves including gap-closure 64-05 (`targetPkgs = p: [ p.zlib ]`) and 64-06 (the genuine-shape re-measurement after a session relaunch). NIX-01..NIX-08 complete; the `ruff`-unrunnable todo filed 2026-08-11 is closed. Next: Phase 65 (`tox-uv-bare` → `tox-uv` revert); Track B (Phase 66) is independent.*

*Last updated: 2026-09-02 — started milestone **v0.9.3 Toolchain and dependency-update repair** via `/gsd-new-milestone`. Scope is the two toolchain defects the owner named — `ruff` unrunnable on NixOS (with its `tox -e py312` companion) and dependabot PRs dying on a stale `uv.lock` — plus a `tox-uv-bare` → `tox-uv` revert the first fix unblocks. **The repair route was chosen on live measurement, not from the todo's candidate list.** That todo proposed `pkgs.ruff` in the devShell, `patchelf`, or system `nix-ld`; the owner asked about FHS instead, and a `buildFHSEnv` probe closed **both** defects at once — an unpatched `ruff` wheel and uv's auto-downloaded generic-linux CPython 3.12.13 both execute inside it, where outside they die on `stub-ld`. Driven end to end on a real worktree provisioned by the documented line, `uv run ruff check .`, `tox config -e py312,lint,type,cov`, `tox -e lint`, `tox -e py312`, `tox -e type` and `tox -e py313` are all green inside FHS and all red or refused outside. The defect is a **class** — generic-linux ELFs that uv installs or downloads — not the `ruff` binary, which is why eight phase summaries record a manual per-worktree shim and why Phase 38-07 misread a forgotten shim as 45 test failures. **Two naive shapes were falsified before being adopted.** `buildFHSEnv`'s `.env` as the devShell does not enter FHS under `nix develop` or `direnv`; and a `shellHook` that `exec`s in fails differently in each — `nix develop` never runs the hook, direnv runs it but the `exec` only replaces its environment-capture subshell. The devShell therefore stays `mkShell` and the FHS is exposed as PATH command shims, which also keeps the documented command strings unchanged and removes the forget-the-prefix failure mode entirely. **Measured costs, all small:** +11 ms per wrapper invocation with no runtime penalty on real workloads (pytest 118 tests and a 1548-test collection both ran *faster* inside FHS than outside), +0.26 GiB of store closure against a 136 GiB store, 4.5 s to build from cache. **CI is deliberately untouched** — no `nix` job, no pinning of the eleven floating `setup-uv@v7` steps — because Windows cannot run nix and its lanes are load-bearing here, and because the FHS wrapper is useless on runners that already have a real loader. The accepted consequence is that `flake.nix`, which this milestone makes load-bearing, keeps zero CI coverage. **Not published:** milestone label v0.9.3, no tag, no PyPI upload, no GitHub Release, `pyproject.toml` held at `0.9.2` with bullets under `## [Unreleased]` — the v0.9.1 precedent — but a PR to `main` is opened and merged, decided up front this time. Phase numbering continues at **64**. Next: define REQUIREMENTS.md, then the roadmap. Prior footer retained below.*

---
*Last updated: 2026-08-31 at the **v0.9.2 milestone close** (`/gsd-complete-milestone`) — full evolution review complete. **v0.9.2 Inline image blocker fix and release SHIPPED**: 2 phases (62–63), 10 plans, 29 tasks, **7/7 v1 requirements complete**, `verified_closeout` — the first since v0.6.4, after seven consecutive `override_closeout`s — and the first milestone in this project's history to complete inside a single calendar day (~11 hours). PyPI `typsphinx 0.9.2` published by release run `33318905691` with every job `success`, `create-release` included; PR #136 merged as `45962faa` with 15/15 checks green; the GitHub Release body **byte-identical** to `scripts/extract_changelog_section.py 0.9.2`'s 4083-byte / 54-line stdout with zero `### Planned for Future Releases` leakage, carrying wheel and sdist; `typsphinx-doc-translations` pinned to `45962faa` via its own dispatched `update-pin.yml` (`fcf66da4`) and tagged `v0.9.2`. **The blocker that cancelled v0.9.1 is fixed and disclosed.** An image not first in its container was emitted adjacent to the preceding code-mode expression, so `-b typstpdf` wrote no PDF for **any** master — true of the published 0.9.0 for its entire life, and undisclosed by D-05. The fix is a 9-line pure insertion routing `visit_image()`/`depart_image()` through the translator's existing separator triad, gated by a real `typst.compile()` over 16 measured failing shapes and 9 that must keep passing, recorded RED against a genuinely restored unfixed tree, with zero pre-existing test edits measured across the 20 files carrying the 144 `image(` matches. 0.9.2's release notes tell 0.9.0 users to upgrade — the first public-surface disclosure this defect ever had. **The close's own carrying lesson: the last gate before a publish is the one that reads the publish's own output.** `63-REVIEW.md` CR-01 and `63-VERIFICATION.md`'s SC#2 block independently caught a false blanket claim inside the curated `## [0.9.2]` section — "the runtime changes are confined to `typsphinx/translator.py`" — *after* the extractor had been run and its structural checks had passed clean, and after all four plans had reported complete. The same evidence file's own invariant sweep held the five-file diff that falsified it. Structural correctness of a release note is not the same property as its claims being true. **The checkbox fence held a second consecutive time, on a requirement that was meant to close**: REL-11's SHA-256 was verified at four separated observations including one taken after all `phase.complete`-family tooling had run, every plan declared `requirements-completed: []` for REL-09, and the three REL boxes were checked by the operator only once PyPI carried `0.9.2`. **REL-04 closed on evidence this release generated** — `create-release` green on a real tag push for the third consecutive time — retiring a row open since 2026-08-04. Roadmap, requirements and research archived to `milestones/v0.9.2-*`, phase directories to `milestones/v0.9.2-phases/`, `REQUIREMENTS.md` removed via `git rm` for the next milestone. **Not verified at this close, by owner selection:** Read the Docs `stable` on `en`/`ja` — an expectation here, not an observation; the procedure is `63-HANDOFF.md` § 5. No `MILESTONE-AUDIT.md` was produced, recorded as an absence rather than claimed as a pass. Phase numbering continues at **64**. Next: `/gsd-new-milestone`. Prior footer retained below.*

---
*Last updated: 2026-08-30 — **Phase 63 (v0.9.2 Release Prep, prep-only) complete and verified 5/5**, 6 plans across 5 waves (4 planned + 2 gap-closure), REL-09 / REL-10 / REL-11 all still deliberately OPEN. **The milestone is prepped, not published**: the tree is at 0.9.2 in `pyproject.toml`, `uv.lock` and `README.md`, the `## [0.9.2]` CHANGELOG entry curates v0.9.1's accumulated bullets together with this milestone's blocker fix, and `63-HANDOFF.md` is a standalone checklist for `/gsd-complete-milestone`. Zero irreversible action taken, re-probed three separate times with positive controls: no `v0.9.2` tag local or remote, no PyPI publish, no GitHub Release, no PR, no `release.yml` run. **The phase's first verification FAILED and the failure was the interesting result.** SC#2's bar is "the extracted release body read rather than assumed"; the original inspection reached the body's *shape* and stopped, and so shipped a checkable falsehood — the `## [0.9.2]` intro claimed the release's runtime changes were "confined to `typsphinx/translator.py`" while `git diff --stat v0.9.0..HEAD -- typsphinx/` shows **five** files. That text publishes byte-identically as the GitHub Release body and the Read the Docs changelog page, where a reader auditing the path-handling fixes would have been misdirected away from `builder.py`, `pathfmt.py` and `template_registry.py`. Gap closure (63-05, 63-06) deleted the blanket claim and re-scoped a **measured** version onto the IMG-08/09/10 bullet, where `git diff --stat e3399825..dd385436 -- typsphinx/` (translator.py alone, 23 insertions) makes it true — then re-took every phase-close observation against the corrected tree, because a claim taken before the correction is a claim about a tree that no longer exists. **The `phase.complete` checkbox flip landed for a sixth consecutive release-prep close, and was caught and reverted.** The fence widened its own catch: where the five prior closes recorded REL-09 alone as the target, this run's whole-file SHA-256 showed `phase.complete` flipping **REL-09, REL-10 and REL-11** together, all three against every plan's `requirements-completed: []`. The flip is line-count-neutral, so the digest — not `wc -l` — is the probe that catches it. Reverted with `git checkout` and never committed in the flipped state; the full before/after probe set is recorded in `63-CLOSEOUT-GUARD.md` § "Third observation". Next: `/gsd-complete-milestone` — the only step that takes irreversible action, and the one that closes REL-09. Prior footer retained below.*

---
*Last updated: 2026-08-30 — **Phase 62 (The `visit_image()` Separator Fix and Its Real-Compile Gate) complete and verified 5/5**, 4 plans across 4 waves, IMG-08 / IMG-09 / IMG-10 / TEST-05 all closed. **The v0.9.2 blocker is fixed and, more to the point, proven to have been necessary.** The production change is nine inserted lines: `visit_image()`'s leading separator triad (`_add_paragraph_separator()` + `_emit_inline_concat_separator()` + the `in_list_item` / `list_item_needs_separator` bookkeeping) is hoisted above the `if self.in_figure:`/`else:` split and `depart_image()`'s trailing bookkeeping is made concat-aware — 0 deletions, both `in_figure` branch bodies byte-identical to the base SHA. **A locked decision was falsified by measurement before planning finished and amended rather than followed**: the originally-recommended branch-scoped triad leaves 4 of 18 masters red, so the hoist-above-the-split form was adopted with owner acknowledgement; rows 9, 10 and 14 of the trigger census are exactly the shapes that would otherwise still be broken, and they are now bound by name. **The gate is the deliverable, not the fix.** Nine string-level image tests existed and this defect shipped in 0.9.0 straight past all of them, so the phase built an 18-master / 27-document fixture project (16 FAIL shapes, one master each, from the measured `FEATURES.md` § Q1 census; 9 PASS shapes under one parent; one image-free root) driven by a single `sphinx-build -b typstpdf` with a `%PDF` magic-byte check on every master — so no structural string assertion in the module can pass vacuously. **RED was established against a genuinely restored unfixed tree, not remembered**: `translator.py` was restored from the phase base SHA `5a837238`, the identical `expected semicolon or line break` refusal reproduced across 17 masters and transcribed verbatim, the fix restored, and `git status --porcelain` recorded empty to prove the restore was byte-identical — and the verifier re-ran that entire choreography independently rather than reading the evidence file. The nine must-keep-passing shapes are pinned to committed goldens; the single measured non-byte-identical shape (image first in its paragraph) is pinned to an **exact committed delta** — one added blank line, with its own `.pre_fix.typ` golden — rather than waived. **Two claims were converted from assertions to observations at phase close.** `git diff --name-status` over the phase range scoped to `tests/` returns 40/40 `A` entries and zero `M`, so the zero-test-edit invariant (D-13) is measured, not promised; and SC#5's authority run is one dispatched CI run (`33302087913`) waited to completion — `windows-latest` and `macos-latest` green individually in both Python lanes, `ruff`'s verdict taken from that run's lint step rather than from this machine, which matters because this project has recorded three defect classes local runs structurally cannot see. That audit was deliberately placed in its own wave, after the work it audits, because a prior phase recorded an audit co-located with its subject abstaining on its own criterion. Full suite 1547 passed / 1 skipped; `black` and `mypy` clean. Code review: 0 blockers, 2 warnings, 2 info — the warnings are the already-pinned blank-line delta (WR-01, a consequence of the pinning decision, not a defect) and a missing rationale comment on `depart_image()`'s new lines (WR-02, real regression-bait: the unconditional `list_item_needs_separator = True` reads as redundant next to the trailing `"\n\n"` and is a plausible future cleanup target). **WR-02 was fixed at the owner's instruction immediately after the phase closed** (`1adad07f`, comment-only — 14/0 numstat, parsed AST identical across the commit, so no success criterion re-opens); WR-01 stands as the recorded consequence of the pinning decision. `62-SECURITY.md` was not produced — the `secure-phase` capability is active, so `/gsd-secure-phase 62` is outstanding. Next: Phase 63 (v0.9.2 Release Prep, prep-only). Prior footer retained below.*

*Last updated: 2026-08-30 — started milestone **v0.9.2 Inline image blocker fix and release** via `/gsd-new-milestone`. Scope set by the owner to the blocker alone plus the publish; every other carried-forward item was explicitly declined for this round. **Three things were measured at HEAD before scoping, not taken from the todo prose.** (1) The defect is live: `visit_image()` emits `image("{escaped_uri}"` with no preceding separator in both its `in_figure` and else branches, a document containing `Inline substitution |sub| in a sentence.` emits `par({text("Inline substitution ")image("img.png")`, and a real `typst.compile()` of the emitted file answers `expected semicolon or line break`. (2) **It is a single site, not a class** — fourteen inline constructs (`:ref:`, inline literal, emphasis, `:abbr:`, `:kbd:`, `:manpage:`, citation reference, `:term:`, `:index:`, `:guilabel:`, external link, footnote reference, `:math:`, `:download:`) were placed mid-sentence in one probe document and the emitted `.typ` scanned for juxtaposed code-mode calls; exactly one hit was found and it was the image, with footnote/math/download each already emitting a leading `\n`. So this milestone fixes one emitter and does not audit a family. (3) **REL-09 cannot carry forward literally.** The v0.9.1 audit records D-08 as carrying it "with wording unchanged", but its text names *v0.9.1 released to PyPI with a curated `## [0.9.1]` CHANGELOG entry* — unachievable for a release that will never exist. Carried with its obligations intact and its version token read as 0.9.2, put to the owner before PROJECT.md was written. **Two standing hazards apply again**: `phase.complete` has auto-flipped the release requirement to `[x]` at five consecutive release-prep closes and was only stopped in v0.9.1 by a recorded `REQUIREMENTS.md` checksum, so the same fence is required; and `release.yml`'s `create-release` job — green at the v0.8.0 and v0.9.0 closes but the job that failed the v0.7.0 tag push with `uv: command not found` — is exercised for real by this milestone's tag push even though REL-04 was declined as a requirement. Phase numbering continues at **62**. Next: define REQUIREMENTS.md, then the roadmap. Prior footer retained below.*

*Last updated: 2026-08-30 at the **v0.9.1 milestone close** (`/gsd-complete-milestone`) — full evolution review complete. **v0.9.1 Windows path correctness COMPLETE but deliberately NOT PUBLISHED**: 4 phases (58–61), 17 plans, 48 tasks, 10/11 v1 requirements complete, `status: tech_debt` with `fail_gate.triggered: false`. Three latent Windows path defect families closed on the product side — the normalized `_escapes_outdir()` predicate, the end-to-end-safe image escape branch (normalized relocation key, 255-byte bound, `escape_typst_string()` at both emission sites, gated by a real `typst.compile()`), and one delimiter-aware `quote_path()` helper in a new zero-import leaf module carrying all 28 path-valued interpolations across three modules. Every fix RED-first against the unfixed tree; POSIX output proven byte-identical by zero pre-existing test edits, measured against `58-REPR-CENSUS.md` rather than asserted. **The release was then cancelled** for a pre-existing `blocker` unrelated to this milestone's work (D-02, D-06, D-07): an inline image not first in its paragraph aborts the Typst compile so `-b typstpdf` writes no PDF — measured byte-identical to the `v0.9.0` tag. No tag, no PyPI publish, no GitHub Release; the next published release is 0.9.2, and REL-09 carries forward unmet with its wording unchanged (D-08). The branch was then merged to `main` as `9db2274c` via merge-only PR #135 (15/15 green) at owner instruction, overriding D-12's no-PR rule but not D-02's no-release rule, so 0.9.2 can carry the fixes. First close in this project's history to publish nothing, and the first in seven with a `MILESTONE-AUDIT.md`. 16 open artifacts acknowledged and deferred, the inline-image blocker among them. Next milestone not yet scoped — `/gsd-new-milestone`, whose first requirement is already known.*

*Last updated: 2026-08-29 — **Phase 59 (Path-Shape Predicate and Image-URI Correctness) complete and verified**: 5 plans in 5 sequential waves, verification `passed` 5/5 must-haves, 0 gaps, 0 human-verification items. All five requirements closed (PATH-01, IMG-04, IMG-05, IMG-06, IMG-07). Product change is confined to `typsphinx/builder.py` and `typsphinx/translator.py`; **zero pre-existing test edits**, measured rather than claimed — `git diff --name-status $PHASE_BASE_SHA..HEAD -- tests/` yields eight `A` lines and nothing else, re-measured after every subsequent commit including the two post-plan fixes. **Two locked decisions were falsified by measurement and amended in place, both owner-approved only after the orchestrator reproduced the measurement itself**: `59-CONTEXT.md` D-01 and ROADMAP SC#2 each predicted `path must not contain a backslash` for the unfixed tree; the measured refusal is `unclosed delimiter`, because a literal carrying a raw backslash AND a raw unescaped `"` has its string terminated at parse time before Typst's semantic backslash check runs — D-01's four probe compiles had each carried exactly one defect, so none of them exercised the literal the real pipeline emits. The substantive claim survived unchanged and was measured true (unfixed, key-normalization-only and escaping-only all fail; only the both-halves tree compiles), and both readings are judged separately in `59-VERIFICATION.md` rather than collapsed. **The plan set was not sufficient on its own — two real defects were found after all five plans reported complete, by two different gates.** Code review found CR-01 (blocker): `_bound_relocation_component()` reserved one *byte* for the stem, which is equivalent to one character only for ASCII, so a multi-byte leading character was dropped entirely by the UTF-8 boundary walk-back while the lower-priority extension kept its allotment — a direct violation of the "never an empty basename" property SC#3 names and the function's own `# D-07: never empty` comment. Fixed in-phase (`924f21d8`) with the reserved unit changed to one character, the shortfall borrowed back from the extension, the duplicated `stem_budget` formula (WR-01) hoisted to one computation, and two regression gates proven RED against the pre-fix tree before the fix was committed. CI found the other: run 1 failed on `ruff` UP012 alone while all six matrix test jobs — both `windows-latest` included — passed. **That failure also corrects a standing note**: ruff is not unrunnable on this dev machine outright; the main `.venv` holds a working wheel and ran clean, but every freshly `uv sync`ed worktree — where all five executors ran — cannot exec it, so no executor in this phase could have caught UP012 locally. CI genuinely is the lint authority, for a more specific reason than previously recorded. Run 3 (`924f21d8`) is the acceptance run at 12/12, dispatched fresh on the post-fix tip. **Tracking automation clobbered prose again, on every call**: `roadmap.update-plan-progress` (×5), `state.begin-phase` and `phase.complete` each orphaned wrapped continuation lines and injected blank lines into bullet lists, and `phase.complete` again left the Current Position progress line contradicting its own frontmatter and the Resume pointer aimed at the finished phase. Every output was diffed and hand-repaired before commit — the second consecutive phase where this was necessary on 100% of calls. Two todos closed (`2026-08-16-escapes-outdir-isabs-not-backslash-normalized`, `2026-08-16-track-image-escape-branch-basename-not-normalized`). Next: `/gsd-plan-phase 60`. Prior footer retained below.*

*Last updated: 2026-08-28 — **Phase 58 (`repr()`-Format Decoupling, test-side only) complete and verified**: 3 plans in 3 sequential waves, verification `passed` 5/5 must-haves, 0 gaps, 0 human-verification items; code review 0 critical / 0 warning / 1 info. MSG-01 closed. **No product change — `git diff --name-only 3b0f2b93..HEAD -- typsphinx/` is empty**, re-measured independently by the orchestrator and again by the verifier, so the next two phases' evidence is unconfounded. The two pass criteria MSG-01 names now route through `tests/_path_naming.py`'s `path_named_in()` — a leaf module with zero `typsphinx` imports whose rule is a full-value match (raw value in text, or `repr()` of it in text), not a basename or substring weakening; the reviewer traced the D-03 same-basename-sibling trap by hand to confirm that. **Each rewrite was proven non-tautological by a real falsification, not by argument**: the path field was temporarily dropped from `builder.py:695-698` and from `builder.py:1766-1769` while a same-shaped sibling field stayed interpolated, each recorded RED, each reverted inside its own task with an empty `git status --porcelain typsphinx/` recorded before the commit. `tests/test_repr_census_guard.py` locks the census by re-deriving it at run time (see the new Key Decisions row), and was itself falsified once to prove it is load-bearing. **`gsd/v0.9.1-windows-path-correctness` is on `origin` with tracking** (SC#5) with no `gsd/v0.9.1-milestone` decoy and no `v0.9.1*` tag — pushed from a worktree, so origin sits at `3bce62b7` while the local tip carries 58-03's own commits; the plan anticipated that lag in writing and SC#5's wording ("pushed and tracking") is satisfied by it. `ci.yml`'s triggers are scoped to `main`/`develop`, so this push dispatched no CI and Phase 58 claims none. **Two carrying notes for Phases 59-61.** First, the census's third bucket: `TestWindowsPathEscapingRegressionGuard._assert_no_doubled_separator` is path-valued AND format-asserting **by design** — it is the inverse of MSG-01's target and MSG-02's own gate depends on it, so Phase 60 must leave it alone; it is written down in `58-REPR-CENSUS.md` rather than left to memory. Second, `_resolve_target_stem()` is invoked **3× per build for the same docname**, so the OUT-02 escape warning is emitted three identical times — 58-01's plan had specified an exactly-one-line assertion and the executor corrected it against the measured product rather than against the plan text. **Tracking automation clobbered prose again, on every single call**: `roadmap.update-plan-progress` (×2), `state.begin-phase` and `phase.complete` each orphaned wrapped continuation lines and injected blank lines into bullet lists, and `phase.complete` additionally left the Current Position progress line contradicting its own frontmatter and the Resume pointer aimed at the finished phase. Every output was diffed and hand-repaired before commit. Next: `/gsd-discuss-phase 59`. Prior footer retained below.*
...
*Last updated: 2026-08-16 — **Phase 56 (Per-Document Template Documentation) complete**: 5 plans across 3 waves, verification `passed` 8/8 must-haves with 0 behavior-unverified, UAT 1/1 passed, `56-SECURITY.md` `threats_open: 0`. DOC-15/DOC-16/DOC-17 all closed — `configuration.rst` now defines `typst_documents` element [4] as the registry key (retracting "accepted and ignored"), documents `typst_document_templates` end to end (per-key semantics, the template-xor-package rule, the reserved `"typst"` key, a nine-shape "When the Build Stops" error catalogue, and the CONF-18 key-naming rules), and publishes migration guidance for the three removed values; `templates.rst`'s asset example and `advanced.rst`'s `refs.bib` guidance now describe the bundle layout that actually ships. Docs-only: zero lines under `typsphinx/`; the new `tests/` work is doc↔code binding gates, not behaviour change. **Three carrying lessons.** **A locked decision was falsified by measurement and amended in place rather than shipped as written**: D-03 asserted a hand-run `typst compile` of a wrapper now needs `--root <outdir>`, which is measurably false for a bare target (Typst default-roots to the compiled file's own directory = `outdir`); only a target carrying a path component needs it, so the published note ships conditionally with **both** branches pinned by real `typst.compile()` calls. **A green gate is not proof a gate is load-bearing**: the verifier ran three live falsification tests — reintroducing the retracted phrase, a stale `_template.typ` claim, and an undocumented `raise ExtensionError` shape — and confirmed each turned its own gate RED before accepting 8/8, restoring a clean tree after each; the error catalogue's gate is AST-based in both directions because one raise site goes through a shared helper and another uses implicit string concatenation. **An "anywhere under X" criterion is settled by the discovery-time repo-wide grep, not the CONTEXT's enumerated floor**: SC#4's sweep surfaced two hits beyond the floor (`CLAUDE.md` still naming the deleted `_write_template_file`; an `examples/charged-ieee` `conf.py` claiming `typst_package` skips an artifact that no longer exists). The phase's code review returned 0 BLOCKER / 2 WARNING, both measured pre-existing and outside SC#4's class — stale `Python 3.9` / `Sphinx 5.0` prerequisites on four published surfaces and a dead `../../docs/configuration.rst` link — so the 8/8 stands and, by owner decision, they were filed forward as one todo rather than folded into the phase. Next: Phase 57 (v0.9.0 Release Prep, prep-only). Prior footer retained below.*

*Last updated: 2026-08-16 — **Phase 55 (v0.8.0-Derived Defects) complete**: 4 plans across 3 waves, verification `passed` 5/5 must-haves, all five v0.8.0-derived requirements (XREF-05, BLD-07, BLD-08, BLD-09, IMG-03) closed on the product side. Suite 1349 → 1366 passed / 5 skipped / 0 failed; `black`/`ruff`/`mypy` clean; regression gate over 47 Phase-53/54/54.1 test files 529 passed. Binding constraint #6 (amended RED) discharged for all five and checked against git history with `git merge-base --is-ancestor`, not against the RED-EVIDENCE files' own prose. **Two carrying lessons.** **A success criterion contradicted itself and was amended on measurement rather than silently satisfied**: ROADMAP SC#4 named a predicate literally while requiring, in the same sentence, a behaviour that predicate measurably does not produce — it evaluates `False` for exactly the driveless-absolute and UNC shapes BLD-09 needs to reach the rehome branch. Plan `55-03` halted at a blocking decision checkpoint instead of resolving it; the owner selected the backslash-normalized spelling on the re-measured table and approved amending SC#4, which `55-04` applied, so shipped code and roadmap text now agree and the amendment traces to its evidence. **A code-review blocker survived its mechanism check but failed its framing check**: CR-01's `path must not contain a backslash` compile failure reproduces exactly as reported, but running the pre-fix `builder.py` (`40b92fc6`) side by side showed the identical failure predates this phase — pre-existing, not a regression, falsifying neither SC#4 nor SC#5. Re-rated to warning and, by owner decision, filed as a pending todo (carrying CR-01, WR-01's missing filename-length bound, and the unescaped `image("...")` emission site together) rather than fixed in-phase, since an in-phase fix would ship product code that never passed this phase's own RED-first, real-compile-gate discipline. The todo requires whatever fix lands to carry a real `typst.compile()` gate — neither BLD-09 test renders or compiles its result, which is why this survived the suite. Five todos resolved by this phase closed; two follow-up todos remain open in the same family. Next: Phase 56 (Per-Document Template Documentation). Prior footer retained below.*
