...
**In progress: v0.6.0 — real-world robustness.** Phases 11–12 of 15 complete (2026-07-12): the Issue #114 figure/image fatal bugs are fixed with a graceful-degrade net and a standing real-`typst.compile()` acceptance gate (Phase 11), and the ten highest-frequency previously-dropped nodes now render correctly and compilably (Phase 12 — versionmodified, `refid` cross-refs, autodoc `desc_*`, transition/glossary/tabular_col_spec/abbr). Remaining: shared-dispatch title/line blocks (13), footnotes pre-pass (14), full-corpus validation (15).
...
Building toward v0.6.0 — see `.planning/REQUIREMENTS.md` for scoped REQ-IDs. **Phase 11 (Issue #114 figure/image fatal fixes + graceful-degrade net + real-compile gate) and Phase 12 (the ten high-frequency independent node handlers — versionmodified, `refid` cross-refs, autodoc `desc_*`, transition/glossary/tabular_col_spec/abbr) are complete.** Remaining: Phase 13 (shared-dispatch `visit_title` generalization for topic + line/line_block, with admonition-title regression fixtures — BLK-02/03), Phase 14 (footnotes via a doctree pre-pass — FN-01, the one architecturally-new item), and Phase 15 (full-corpus `-b typstpdf` validation of Sphinx's own `doc/` tree — GATE-02).
...
---
*Last updated: 2026-07-12 — Phase 12 (high-volume independent node handlers) completed: 10/10 requirements validated, render-gate suite now 8 real-compile classes. v0.6.0 now 2/5 phases done (11–12). Milestone v0.6.0 (real-world robustness) was started 2026-07-11 via `/gsd-new-milestone`. Scope: Issue #114 figure/image fatal-bug fix + high-frequency dropped-node support so Sphinx's own docs compile through `typstpdf`. Prior footer retained below for history.*
